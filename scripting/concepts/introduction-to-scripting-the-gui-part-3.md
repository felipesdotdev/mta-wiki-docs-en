---
doc_id: "mta-wiki:5037"
title: "Introduction to Scripting the GUI - Part 3"
source_title: "Introduction to Scripting the GUI - Part 3"
source_url: "https://wiki.multitheftauto.com/wiki/Introduction_to_Scripting_the_GUI_-_Part_3"
revision_id: 60326
language: "en"
categories: ["GUI_Tutorials"]
generated_at: "2026-07-26T16:15:53.353322+00:00"
---

# Introduction to Scripting the GUI - Part 3
...
For this tutorial we will be using one [window](mta://scripting/concepts/element-gui-window.md), three [buttons](mta://scripting/concepts/element-gui-button.md) and one [label](mta://scripting/concepts/element-gui-text-label.md).
We will be using **absolute** position values.

As noted in [Previous tutorials](https://wiki.multitheftauto.com/wiki/Category:GUI_Tutorials), all the GUI must be made client side.
...
To create a window we will use [guiCreateWindow](mta://scripting/client/functions/guicreatewindow.md):
...
To create a label we will use [guiCreateLabel](mta://scripting/client/functions/guicreatelabel.md):
...
To create a button we will use [guiCreateButton](mta://scripting/client/functions/guicreatebutton.md):
...
For this tutorial, we will use a simple [command](mta://scripting/shared/functions/addcommandhandler.md).
...
so open up your serverside 'script.lua' file (from the [Introduction to Scripting](mta://scripting/concepts/scripting-introduction.md)) or another suitable serverside file to work with.

On the clientside, we are triggering the server event "movePlayerToPosition". So, we will first define that event.
To do that we will use [addEvent](mta://scripting/shared/functions/addevent.md), then [addEventHandler](mta://scripting/shared/functions/addeventhandler.md):
...
Note the use of [spawnPlayer](mta://scripting/server/functions/spawnplayer.md) rather than, for example, [setElementPosition](mta://scripting/shared/functions/setelementposition.md).
Using [spawnPlayer](mta://scripting/server/functions/spawnplayer.md) has several advantages, such as being able to set the position when 'de-spawned' or dead, and automatically refilling health.
Of course, [setElementPosition](mta://scripting/shared/functions/setelementposition.md) would work just as well to simply move them, so the choice is yours.
