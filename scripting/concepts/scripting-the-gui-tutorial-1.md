---
doc_id: "mta-wiki:5023"
title: "Scripting the GUI - Tutorial 1"
source_title: "Scripting the GUI - Tutorial 1"
source_url: "https://wiki.multitheftauto.com/wiki/Scripting_the_GUI_-_Tutorial_1"
revision_id: 60911
language: "en"
categories: ["GUI_Tutorials"]
generated_at: "2026-07-26T16:16:36.371621+00:00"
---

# Scripting the GUI - Tutorial 1
...
**Note that this tutorial assumes you are familiar with all the content covered in the [Previous tutorial](mta://scripting/concepts/introduction-to-scripting-gui.md).**

## Setting up the Vehicle Selection

### Creating the GUI

To begin, open up a clientside lua file (if you have been following the [Scripting Introduction](mta://scripting/concepts/scripting-introduction.md), this is gui.lua) to work with.

In this file, we will start writing our function for creating the GUI.
As covered in the [Previous tutorial](mta://scripting/concepts/introduction-to-scripting-gui.md), there are 2 value type options available when creating gui: **relative** and **absolute**.
...
We now also need to call this function somewhere, otherwise the GUI will never be created.
As in previous tutorials, to do this we can use [onClientResourceStart](mta://scripting/client/events/onclientresourcestart.md).
...
Unlike in previous tutorials, we want players to be able to open this window whenever they chose, not when the resource starts. 
So, we can add a [Command](mta://scripting/shared/functions/addcommandhandler.md) to do this.
...
To begin with, we will do this using a simple clientside table. 
...
The table contains vehicles in the format ["description"] = vehicle_id :
...
We will also set hidden data using [guiGridListSetItemData](mta://scripting/client/functions/guigridlistsetitemdata.md) to store the vehicle id:
...
We have attached the [onClientGUIClick](mta://scripting/client/events/onclientguiclick.md) event to buttonCreate already, so now we need to write the function that it calls.
In this function we do some basic error checking, such as making sure a vehicle has been selected in the list. 
We can then use some maths to find a position infront of the player and send the server this information to spawn the vehicle (using [triggerServerEvent](mta://scripting/client/functions/triggerserverevent.md))
...
On the server side, we will first of all need to define the custom event that we triggered before from the client. This can be done using [addEvent](mta://scripting/shared/functions/addevent.md) and [addEventHandler](mta://scripting/shared/functions/addeventhandler.md).
...
If you are unsure of xml data structures, you can browse the [MTA xml functions](mta://scripting/concepts/server-scripting-functions.md) for help.
...
Note the use of for loops in the code with [xmlNodeGetChildren](mta://scripting/shared/functions/xmlnodegetchildren.md). This saves us a lot of time and effort as we do not have to manually code each group and vehicle into the list.
...
To enable the player to simply double click on a group name to expand/collapse it we need to use the [onClientGUIDoubleClick](mta://scripting/client/events/onclientguidoubleclick.md) event.
...
This completes the second section of the tutorial.

For further GUI tutorials, see [Tutorial 2 (Gates and Keypads)](mta://scripting/concepts/scripting-the-gui-tutorial-2.md)
