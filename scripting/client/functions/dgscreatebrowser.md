---
doc_id: "mta-wiki:10168"
title: "DgsCreateBrowser"
source_title: "DgsCreateBrowser"
source_url: "https://wiki.multitheftauto.com/wiki/DgsCreateBrowser"
revision_id: 71425
language: "en"
categories: ["Client_functions", "Utility_templates"]
generated_at: "2026-07-26T16:12:12.575761+00:00"
---

# DgsCreateBrowser

This function creates a browser handled and rendered by dgs.

All of the functions of browser are available.

| [[{{{image}}}\|link=\|]] | Note: You have to use http://mta/resourceName/html/index.html instead of http://mta/local/html/index.html , because the local is relative to the resource that creates the browser. DGS browser is created in DGS, this means local will be direct to DGS . |
| --- | --- |
|  |  |
...
```
element dgsCreateBrowser ( float x, float y, float width, float height, bool relative [, element parent = nil, bool isLocal = false, bool isTransparent = false, float resolutionX = width, float resolutionY = height, int color = 0xFFFFFFFF ] )
```

### Required Arguments

- **x:** A float of the 2D x position of the DGS browser on a player's screen.  This is affected by the *relative* argument.

- **y:** A float of the 2D y position of the DGS browser on a player's screen. This is affected by the *relative* argument.

- **width:** A float of the width of the DGS browser. This is affected by the *relative* argument.

- **height:** A float of the height of the DGS browser. This is affected by the *relative* argument.
...
- **isLocal:** Sets whether the browser can only show local content or content from the internet. (see [createBrowser](mta://scripting/client/functions/createbrowser.md))

- **isTransparent:** true if you want the browser isTransparent, false for opaque. (see [createBrowser](mta://scripting/client/functions/createbrowser.md))

- **resolutionX:** The browser's native width. (see [createBrowser](mta://scripting/client/functions/createbrowser.md))

- **resolutionY:** The browser's native height. (see [createBrowser](mta://scripting/client/functions/createbrowser.md))
...
local webBrowser = DGS:dgsCreateBrowser(200,200,400,400,false)
...
local webBrowser = DGS:dgsCreateBrowser(200,200,400,400,false,nil,true)

--The event onClientBrowserCreated will be triggered, after the browser has been initialized.
--After this event has been triggered, we will be able to load our URL and start drawing.
addEventHandler("onClientBrowserCreated", webBrowser, function()
	--After the browser has been initialized, we can load a local website
	loadBrowserURL(webBrowser, "http://mta/"..getResourceName(resource).."/index.html" ) --Note: you need to specify the resource name in the url.
end)
```
