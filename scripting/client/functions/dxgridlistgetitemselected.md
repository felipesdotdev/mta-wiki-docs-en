---
doc_id: "mta-wiki:13692"
title: "DxGridListGetItemSelected"
source_title: "DxGridListGetItemSelected"
source_url: "https://wiki.multitheftauto.com/wiki/DxGridListGetItemSelected"
revision_id: 74651
language: "en"
categories: ["Client_functions"]
generated_at: "2026-07-26T16:14:49.937326+00:00"
---

# DxGridListGetItemSelected

Esta función sirve para obtener la fila de articulos/items seleccionada de una [dxGridList](mta://scripting/client/functions/dxgridlist.md).
...
```
int table dxGridListGetItemSelected( element element )
```

### Argumentos requeridos

- **element**: El elemento [dxGridList](mta://scripting/client/functions/dxgridlist.md).

### Retorno

- **int**: El numero de la fila seleccionada.

- **table**: Una tabla con todos los articulos/items de la fila seleccionada
...
--creamos una lista
lista = dxGridList( 588, 349, 351, 120 )

--Agregamos 3 columnas a la lista...
for _, c in ipairs({{'Vehiculo', 2}, {'Dueño', 2}, {'Costo', 3}}) do
   dxGridListAddColumn(lista, c[1], c[2])
end

-- Agregamos 2 filas de items, hay 3 valores en cada función porque solamente se crearon 3 columnas ↑
dxGridListAddItem(lista, 'Infernus', 'Claw', '50000')
dxGridListAddItem(lista, 'Sultan', 'Pand', '50000')

boton = dxButton( 276, 80, 100, 40, 'Clickea')
addEventHandler ( "onClick", boton,
   function()
      local inx, tab = dxGridListGetItemSelected(lista)
      if inx ~= -1 then
         print(inspect(tab))
      end
   end
