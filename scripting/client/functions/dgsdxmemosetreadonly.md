---
doc_id: "mta-wiki:9939"
title: "DgsMemoSetReadOnly"
source_title: "DgsDxMemoSetReadOnly"
source_url: "https://wiki.multitheftauto.com/wiki/DgsDxMemoSetReadOnly"
revision_id: 53574
language: "en"
categories: ["Client_functions"]
generated_at: "2026-07-26T16:13:11.739192+00:00"
---

# DgsMemoSetReadOnly

This function allows you to set or remove read-only status for a dgs memo. If read-only is set to *true*, the box is not editable.

## Syntax

```
bool dgsMemoSetReadOnly ( element memo, bool status )
```

### Required Arguments

- **memo:** The dgs-dxmemo that you want to apply.

- **status:** A boolean value indicating whether read-only is to be enabled or disabled.

### Returns

Returns *true* if memo field's read-only status was changed successfully, *false* otherwise.
...
function createAdvert( )
    local advert = DGS:dgsCreateMemo( 0.845, 0.94, 0.15, 0.05, myWebsite, true ) -- create memo field for the advert
    if advert then -- if it was successfully created
        DGS:dgsMemoSetReadOnly( advert, true ) -- make it read only
    end
end
