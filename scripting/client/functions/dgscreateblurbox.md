---
doc_id: "mta-wiki:11737"
title: "DgsCreateBlurBox"
source_title: "DgsCreateBlurBox"
source_url: "https://wiki.multitheftauto.com/wiki/DgsCreateBlurBox"
revision_id: 74519
language: "en"
categories: ["Client_functions"]
generated_at: "2026-07-26T16:12:12.276832+00:00"
---
...
```
element dgsCreateBlurBox( int resolutionX, int resolutionY [, texture sourceTexture = nil ] )
```
...
- **resolutionX**: A number of the resolution of the blur box in width.

- **resolutionY**: A number of the resolution of the blur box in height.

### Optional Argument

- **sourceTexture**: A texture that you want to blur. Set to *nil* to blur the screen.

### Returns

Returns a dgs-dxblurbox element (DGS Plugin Type)[ shader (Element Type) ] if succeed, *false* otherwise

## Example

This example shows creating a blur box on image

```
DGS = exports.dgs --get exported functions from dgs
local x,y,width,height = 200,200,600,500

local blurbox = DGS:dgsCreateBlurBox(width, height) --Blur Box
local blurArea = DGS:dgsCreateImage(x,y,width,height,blurbox,false) --Blur Box Renderer
local window = DGS:dgsCreateWindow(x,y,width,height,"Test Window",false) --Window
DGS:dgsAttachToAutoDestroy(blurbox,window) --Destroy the blurbox with window
DGS:dgsAttachToAutoDestroy(blurArea,window) --Destroy the blurArea with window
DGS:dgsAttachElements(blurArea,window,0,0,1,1,true,true) --Attach Blur Box Renderer to Window
DGS:dgsSetLayer(blurArea,"bottom") --Make sure the blurArea is always under window
```

This example adds a blurbox on the full screen
...
This example shows that you can blur a dgs element

```
loadstring(exports.dgs:dgsImportFunction())()-- load functions

local sW, sH = guiGetScreenSize() -- get the screen size
local blurbox = dgsCreateBlurBox(sW/2, sH/2) -- Create a blur Box
dgsSetProperty(blurbox,"updateScreenSource",true)
dgsBlurBoxSetIntensity(blurbox,1)
dgsBlurBoxSetLevel(blurbox,15)
local blurArea = dgsCreateImage(0,0,0.5,0.5,blurbox,true) -- --Blur Box Renderer
window = dgsCreateWindow(400,200,800,200,"123",false)
dgsSetPostGUI(window,false)
dgsSetLayer(window,"bottom") --This window should be always under the blur box
```
