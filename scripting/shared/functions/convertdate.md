---
doc_id: "mta-wiki:13225"
title: "ConvertDate"
source_title: "ConvertDate"
source_url: "https://wiki.multitheftauto.com/wiki/ConvertDate"
revision_id: 71148
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:35.401101+00:00"
---

# ConvertDate

This function converts time into a different look

## Syntax

```
bool convertDate( string data, bool convertTime )
```

### Required Arguments

- **date**: String of date(YYYY-MM-DD HH:MM:SS).

- **convertTime**: True/False if the function should also convert the time.
...
function convertDate(string,convertTime)
	if string then
		local format={}
		for _,v in ipairs(split(string,"-",""))do
			table.insert(format,v)
		end
		local date=gettok(format[3], 1, string.byte(' ') ).." "..months[tonumber(format[2])].." "..format[1]
		if convertTime then
			local format={}
			for _,v in ipairs(split(string,":",""))do
				table.insert(format,v)
			end
			date=date.." "..gettok(format[1], 2, string.byte(' ') )..":"..format[2]..":"..format[3]
		end
		return date
	end
end
...
```
addCommandHandler("getformattime",function(plr)
	local time = getRealTime()
        local formattime = string.format("%04d-%02d-%02d %02d:%02d:%02d", time.year + 1900, time.month + 1, time.monthday, time.hour, time.minute, time.second)
	local result=convertDate(formattime)
	outputChatBox(result,plr)
end)
```
...
```
string="2020-05-01 19:53:02"

addCommandHandler("getformattime",function(plr)
	local result=convertDate(string,true)
	outputChatBox(result,plr)
end)
```

01 may 2020 19:53:02
