---
doc_id: "mta-wiki:12341"
title: "GetResourceSize"
source_title: "GetResourceSize"
source_url: "https://wiki.multitheftauto.com/wiki/GetResourceSize"
revision_id: 73021
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:23.793760+00:00"
---

# GetResourceSize

This function returns the size of a specified resource in **kB**(kilobyte). Resource files that are not declared in [meta](mta://reference/misc/meta.md) are not counted.

## Syntax

```
float getResourceSize ( string/resource theResource )
```

### Required Arguments

- **theResource**: A [string](mta://reference/misc/string.md) containing the name of the resource or the [resource](mta://reference/misc/resource.md) object (non-element) to obtain the size.

### Returns

Returns a [float](mta://reference/misc/float.md) size of the resource counting the meta. Otherwise it returns *false*.
...
This first example creates a command handler with the following syntax **/viewsize <resource_name>**.
...
function resSize(player, cmd, resName)
    if (resName) then
        local size = getResourceSize(tostring(resName))
        if (size) then
            outputChatBox("The size of resource '"..tostring(resName).."' is "..tostring(size).."kB", player)
        else
            outputChatBox('Could not get a size, check the specified arguments.', player)
            outputChatBox('Possible mistakes: \nArg#1 is not a string or userdata\nResource with the specified name was not found', player)
        end
    else
        outputChatBox('Specify a name of a resource', player)
    end
end
addCommandHandler('viewsize', resSize)
```

This second example creates a command handler with the following syntax **/sizeofthis** which output in chatbox the size of the resource on which the command was executed.
...
function resSize(player, cmd)
    local size = getResourceSize(getThisResource())
    if (size) then
        outputChatBox("The size of this resource("..getResourceName(getThisResource())..") is "..tostring(size).."kB", player)
    else
        outputChatBox('Could not get a size, check the specified arguments.', player)
        outputChatBox('Possible mistakes: \nArg#1 is not a string or userdata\nResource with the specified name was not found', player)
    end
end
addCommandHandler('sizeofthis', resSize)
```
