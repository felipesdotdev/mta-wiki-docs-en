---
doc_id: "mta-wiki:7927"
title: "ConvertServerTickToTimeStamp"
source_title: "ConvertServerTickToTimeStamp"
source_url: "https://wiki.multitheftauto.com/wiki/ConvertServerTickToTimeStamp"
revision_id: 82768
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:12:03.414927+00:00"
---

# ConvertServerTickToTimeStamp

This function converts server ticks to a unix timestamp. **Note**: this was built to work with the server tick timestamp returned by [onPlayerScreenShot](mta://scripting/server/events/onplayerscreenshot.md).
Author: qaisjp
...
```
function convertServerTickToTimeStamp(tick) 
   -- Get the current time (table).
   -- Note: getRealtime() creates a table that consumes memory when called frequently.

   local time = getRealtime()

   -- Calculate the difference between the passed tick and the current one in seconds
   -- and add it to the current Unix time.
   return time.timestamp + ((tick - getTickCount()) * 0.001)

   -- refactoring code
   --return getRealtime().timestamp+((getTickCount()*0.001)-(tick*0.001)); 
end
```
