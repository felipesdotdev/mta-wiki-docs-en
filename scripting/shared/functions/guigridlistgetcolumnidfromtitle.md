---
doc_id: "mta-wiki:10198"
title: "GuiGridListGetColumnIDFromTitle"
source_title: "GuiGridListGetColumnIDFromTitle"
source_url: "https://wiki.multitheftauto.com/wiki/GuiGridListGetColumnIDFromTitle"
revision_id: 60064
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:36.470614+00:00"
---

# GuiGridListGetColumnIDFromTitle

This function gets a gridlist's column index from the column title. This function is particularly useful when the columns in your gridlist are created programmatically using a pairs loop. Pairs are often used to loop an unordered list and is impossible to guess the order of such a list and it may not always be the same.

## Syntax

```
int guiGridListGetColumnIDFromTitle ( element gridlist, string title )
```

## Required Arguments

- **gridlist:**  The gridlist you want to get the column index from.

- **title:**  The title of the column you want to get the index of.
...
- A column *id* of the column matching the title, *false* otherwise .

## Example

- Here's a quick example which grabs the index from the id column and triggers a custom event to delete an account matching that id. (This example is using [OOP](mta://tutorials/oop.md))
...
function deleteAccount()
	local row = accountList:getSelectedItem()
	if row then
		local id = accountList:getItemText(row, guiGridListGetColumnIDFromTitle(accountList, 'id'))
		if id then
...
addCommandHandler('del', deleteAccount)
```
