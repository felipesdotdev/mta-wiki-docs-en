---
doc_id: "mta-wiki:14683"
title: "Wait"
source_title: "Wait"
source_url: "https://wiki.multitheftauto.com/wiki/Wait"
revision_id: 82847
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:17:06.239911+00:00"
---

# Wait

This function pauses the execution of a function for a specific amount of time. It allows you to write sequential, readable code instead of using complex callback structures or multiple timers.

**Note:** This function must be used inside a function wrapped by the `async` helper.
...
- **milliseconds**: The time in milliseconds to pause the execution.
    ...
    return function(...)
        ...
        local function resume(...)
            ...
            if not status and err then
                ...
                outputDebugString("Stack trace: "..debug.traceback(co, err), 1)
            end
        end
...
function wait(ms)
    ...
    if not co then 
        outputDebugString("wait() must be called inside an async function!", 1)
    end
...
addEventHandler("onClientResourceStart", resourceRoot, async(function()
    outputChatBox("Script started!")
    
    wait(1000)

    outputChatBox("1 second has passed.")
    
    wait(2000)

    outputChatBox("3 seconds total.")
end))

--// Example 2

function welcomeToServer()
    fadeCamera(false, 1.0)
    outputChatBox("Loading data..", 0, 255, 0)
    
    wait(1500)

    outputChatBox("Welcome iManGaaX!", 0, 255, 0)
    ...
    outputChatBox("You are now free to move.", 0, 255, 0)
end
async(welcomeToServer)()
...
async(function()
    ...
    outputChatBox("Starting countdown: "..seconds.." seconds..", 255, 255, 0)

    wait(1000)

    for i = seconds, 1, -1 do
        outputChatBox(i, 255, 255, 0)
        wait(1000)
    end

    outputChatBox("Go!", 0, 255, 0)
end)()
...
async(function()
    while true do
        outputChatBox("Hello "..getPlayerName(localPlayer)..", If you need help, contact one of the staff!")
        wait(60000)
    end
end)()
```
...
- wait » This function pauses the execution of a function for a specific amount of time. It allows you to write sequential, readable code instead of using complex callback structures or multiple timers.
