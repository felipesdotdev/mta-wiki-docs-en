---
doc_id: "mta-wiki:10798"
title: "Math.randomDiff"
source_title: "Math.randomDiff"
source_url: "https://wiki.multitheftauto.com/wiki/Math.randomDiff"
revision_id: 76269
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:10.303947+00:00"
---

# Math.randomDiff

This function generates a pseudo-random integer between **start** and **finish** that's always different from the last random number generated.

## Syntax

```
int math.randomDiff (int start, int finish)
```

### Required arguments

- **start:** An integer representing the minimum range. Generated numbers will be equal or higher than it. This can't be equal to **finish**.

- **finish:** An integer representing the maximum range. Generated numbers will be equal or lower than it. This can't be equal to **start**.

### Returns

This function returns a random integer. If invalid arguments are used, it returns *false*.
...
This example shows 10 random numbers between -1 and 2 to the player when he uses */rand* command. Every random number is always different from its antecedent.

```
addCommandHandler ("rand", function ()
	for i=1, 10 do
		outputChatBox (tostring (math.randomDiff(-1, 2)))
	end
end)
```
...
This example shows 10 random numbers between -5 and 8 when he uses */rand* command. Every random number never repeats during all the sequence.

```
function table.find (theTable, data)
...
addCommandHandler ("rand", function (thePlayer)
	local myTable = {}
	for i=1, 10 do
		local numberRand = math.randomDiff(-5, 8)
		while table.find (myTable, numberRand) do
			numberRand = math.randomDiff(-5, 8)
		end
		table.insert (myTable, numberRand)
		outputChatBox (numberRand, thePlayer)
	end
end)
```
