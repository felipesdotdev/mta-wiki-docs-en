---
doc_id: "mta-wiki:13205"
title: "IsPedDiving"
source_title: "IsPedDiving"
source_url: "https://wiki.multitheftauto.com/wiki/IsPedDiving"
revision_id: 73593
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:58.046430+00:00"
---

# IsPedDiving

This function checks if the ped is diving in the water.
...
```
bool isPedDiving( ped thePedToCheck )
```
...
Returns true if ped is diving in water, false if not.
...
```
function isPedDiving(thePedToCheck)
	if isElement(thePedToCheck) then
		if getElementType(thePedToCheck) == "player" or getElementType(thePedToCheck) == "ped" then
			if getPedSimplestTask(thePedToCheck) == "TASK_SIMPLE_SWIM" and getPedTask(thePedToCheck, "primary", 2) == "TASK_COMPLEX_IN_WATER" then
    				local x, y, z = getPedBonePosition(thePedToCheck, 8 )
    				local groundPosition, waterLevel = getGroundPosition(x, y, z), getWaterLevel (x, y, z, true)
    				return (groundPosition+waterLevel)-(groundPosition-z-0.5) < 0
			end
		end
	end
	return false
...
The example shows if the player is diving in the water when he presses b.

```
function checkDiving()
	if isPedDiving(localPlayer) then
		outputChatBox("You are currently diving")
	else
		outputChatBox("You are not currently diving")
	end
end
bindKey("b", "down", checkDiving)
```
