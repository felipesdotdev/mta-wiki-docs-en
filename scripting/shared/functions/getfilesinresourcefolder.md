---
doc_id: "mta-wiki:12486"
title: "GetFilesInResourceFolder"
source_title: "GetFilesInResourceFolder"
source_url: "https://wiki.multitheftauto.com/wiki/GetFilesInResourceFolder"
revision_id: 80593
language: "en"
categories: ["Useful_Functions", "MTA_Wiki:Delete", "Utility_templates"]
generated_at: "2026-07-26T16:15:13.654580+00:00"
---
...
| --- | --- |
| Reason: pathListDir does the job. Actions: Delete (Administrators) - Discuss - What links here - Category |  |

This function lists all files in a resource folder. **PS**: This function should only be performed on server-side.

| [[{{{image}}}\|link=\|]] | Note: The listing is only done with files that are specified in the meta.xml file in attributes named src . |
| --- | --- |
|  |  |
...
- **directory**: The [string](mta://reference/misc/string.md) that will determine the folder in which to list files. To get all files, use `'/'`

### Optional Arguments

*NOTE:* When using optional arguments, you might need to supply all arguments before the one you wish to use. For more information on optional arguments, see [optional arguments](mta://reference/misc/optional-arguments--f0d8694a.md).

- **theResource**: The [resource](mta://reference/misc/resource.md) to be searched. If not specified, the search is performed on the resource where the function was executed.

### Returns

Returns two [tables](mta://reference/misc/table.md), the 1st storing the name / path of the obtained files, and the 2nd storing only the name of the files (with their respective extensions). Otherwise it returns **false**.

###### Format

- 1st table: `{**'Folder1/Folder2/file.ext'**, **'Folder1/file2.ext'**, **'file3.ext'**}`

- 2nd table: `{**'file.ext'**, **'file2.ext'**, **'file3.ext'**}`
...
This example create the command handler **/filesfrom <folder> [resource_name]** which displays in the chat some information about the data obtained from the file search.

```
function searchFiles(player, cmd, folderName, resName)
    if (hasObjectPermissionTo(player, 'function.banPlayer', false)) then -- The player will only proceed with this command if he is allowed to ban
        if (folderName) then -- If you specified the folder name, then
            local theRes
            if (resName) then -- If you specified the name of the resource then we will define it
                theRes = getResourceFromName(tostring(resName))
                if not theRes then -- If there is no resource with the specified name, then we will let you know
                    return outputChatBox("#FF0000##FFFFFF There is no resource named '"..resName.."'", player, 255, 255, 255, true)
                end
            end
            local files, onlyFilesName = getFilesInResourceFolder(folderName, theRes) -- then we will do the search
            if (files and onlyFilesName) then -- If everything went well, then we show the response data
                outputChatBox('===== Got files! =====', player, 255, 255, 255, true)
                outputChatBox('=> Folder: '..tostring(folderName), player, 255, 255, 255, true)
                outputChatBox('=> Resource: '..tostring(resName), player, 255, 255, 255, true)
                outputChatBox('=> Total files: '..tostring(#files)..' arquivos', player, 255, 255, 255, true)
                outputChatBox('=> Files: #00FF00'..tostring(table.concat(onlyFilesName, ', ')), player, 255, 255, 255, true)
                outputChatBox('==============================', player, 255, 255, 255, true)
            else
                outputChatBox("#FF0000##FFFFFF Couldn't get file list", player, 255, 255, 255, true)
            end
        else
            outputChatBox('#FF0000##FFFFFF Specify the name of the folder you want to search for files', player, 255, 255, 255, true)
        end
    else
        outputChatBox('# You are not allowed to run this command.', player, 255, 0, 0, false)
    end
end
addCommandHandler('filesfrom', searchFiles)
```

Click to collapse [-]
Example 2 (Server)

This example create the command handler **/fileshere <folder>** which displays in the chat information about the files obtained by the useful function.

```
function searchFiles(player, cmd, folderName)
    if (hasObjectPermissionTo(player, 'function.banPlayer', false)) then -- Only those who are allowed to ban players will continue with this command.
        if (folderName) then -- If you specified the name of the folder you want to search, then ...
            local files, onlyFilesName = getFilesInResourceFolder(folderName)
            if (files and onlyFilesName) then -- If we obtained data about the files, then we will display
                outputChatBox('===== Got files! =====', player, 255, 255, 255, true)
                outputChatBox('=> Folder: '..tostring(folderName), player, 255, 255, 255, true)
                ...
                outputChatBox('#FF0000##FFFFFF Could not retrieve files in the specified folder', player, 255, 255, 255, true)
            end
        else
            outputChatBox('#FF0000##FFFFFF Specify the name of the folder you want to search', player, 255, 255, 255, true)
        end
    else
        outputChatBox('# You are not allowed to run this command', player, 255, 0, 0, false)
    end
end
addCommandHandler('fileshere', searchFiles)
```

## Credits

- **Author:** [KronoS Lettify](https://wiki.multitheftauto.com/wiki/User:Lettify).
