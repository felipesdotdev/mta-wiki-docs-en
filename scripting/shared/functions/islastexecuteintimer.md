---
doc_id: "mta-wiki:14157"
title: "IsLastExecuteInTimer"
source_title: "IsLastExecuteInTimer"
source_url: "https://wiki.multitheftauto.com/wiki/IsLastExecuteInTimer"
revision_id: 78424
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:56.231405+00:00"
---

# IsLastExecuteInTimer

This function check if the execute is the last execute in the [timer](mta://reference/misc/timer.md).

## Syntax

```
bool isLastExecuteInTimer( timer theTimer )
```

### Required Arguments

- **theTimer**: the [timer](mta://reference/misc/timer.md) that you will check if the execute is the last execute in it.

### Returns

- *true* if the execute is the last execute in the timer.
...
```
function isLastExecuteInTimer(theTimer)
    assert(isTimer(theTimer), "Bad argument @ 'isLastExecuteInTimer' [Expected lua-timer at argument 1, got "..(type(theTimer)).."]")
    return ({getTimerDetails(theTimer)})[2] == 1
end
```

**Authors:** [Youssef Maged (iManGaaX)](https://wiki.multitheftauto.com/wiki/User:IManGaaX), Tracer

## Example

This example will out put in chat 'Timer has Finished!' after 5 executes, Each execute is 1 second (Total: 5 seconds).
...
myTimer = setTimer(function()
    if isLastExecuteInTimer(myTimer) then
        outputChatBox("Timer has Finished!")
    end
end, 1000, 5)
```
