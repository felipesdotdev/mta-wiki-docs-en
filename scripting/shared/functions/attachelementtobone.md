---
doc_id: "mta-wiki:12560"
title: "AttachElementToBone"
source_title: "AttachElementToBone"
source_url: "https://wiki.multitheftauto.com/wiki/AttachElementToBone"
revision_id: 75224
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:24.824500+00:00"
---

# AttachElementToBone

This function allows you to attach an object, ped, player or vehicle to a ped or player bone for **ONE FRAME**. In order to attach an element continuously to a bone, this function should be used in conjunction with [onClientPedsProcessed](mta://scripting/client/events/onclientpedsprocessed.md).
The function checks if a player is actually on screen and if he is not, the object gets moved out of the stream distance in order to preserve some performance.
Unlike bone_attach, this function provides perfect accuracy for the attached elements and performs better.

## Syntax

```
bool attachElementToBone(element theElement, ped thePed, int boneID, float offsetX, float offsetY, float offsetZ, float rotationOffsetX, float rotationOffsetZ, float rotationOffsetZ)
```
...
- **rotationOffsetX**: The rotation X-axis offset from the bone, in **radians**.

- **rotationOffsetY**: The rotation Y-axis offset from the bone, in **radians**.

- **rotationOffsetZ**: The rotation Z-axis offset from the bone, in **radians**.
...
Returns true if the object has successfully been attached to the ped/player, returns false if the specified ped/player is not on screen and the object has been moved out of the streaming distance.
...
This example attaches a sniper rifle and a backpack to the local player.

```
local theGun = createObject(358, 0, 0, 0)
local thePara = createObject(371, 0, 0, 0)

local function onPedsProcessed()
	attachElementToBone(theGun, localPlayer, 24, 0, 0, 0, 0, 0, 0) -- Attach the sniper model to bone #24 of the local player, no offset is needed because the default GTA weapon models fit perfectly fine into bone #24
	attachElementToBone(thePara, localPlayer, 3, 0.1, -0.14, 0, math.rad(90), 0, 0) -- Attach parachute to bone #3 of the local player with a small position and rotation offset, so it looks nice
end
