---
doc_id: "mta-wiki:11175"
title: "IsPlayerStaff"
source_title: "IsPlayerStaff"
source_url: "https://wiki.multitheftauto.com/wiki/IsPlayerStaff"
revision_id: 78498
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:59.951602+00:00"
---

# IsPlayerStaff

This function checks if the player is server staff (for example: Moderator or Admin, by ACL group membership).

It's useful to check if a player is supposed to have access to/be able to use something.

## Syntax

```
bool isPlayerStaff ( element thePlayer )
```

### Required arguments

- **thePlayer**: The player to check rights of

### Returns

Returns *true* if the player is in a staff group, *false* otherwise.
...
This example adds a command to speed up your vehicle's handling, but only when you're in a staff ACL.

**Beginners note**: By adding "**if not isPlayerAllowed(client) then return end**" before execution of the code, you will limit its usage to only server admins and staff.

```
function speedUpHandling(client)
    if not isPlayerStaff(client) then return end -- check if the player is part of a staff ACL, if not then we won't execute (allow them to use it)
    local vehicle = getPedOccupiedVehicle(client)
    if vehicle then
        setVehicleHandling(vehicle,"engineAcceleration",17)
        setVehicleHandling(vehicle,"tractionMultiplier",0.9)
        setVehicleHandling(vehicle,"tractionLoss",1.1)
        setVehicleHandling(vehicle,"dragCoeff",0.0)
        setVehicleHandling(vehicle,"maxVelocity",750)
    end
end
addCommandHandler("tunevehicle",speedUpHandling)

-- Now, we'll add the custom function code to the file, so that the call to 'isPlayerStaff(client)' actually works (in the script that needs access check, like above)
local staffACLs =
{
    aclGetGroup("Admin"),
    aclGetGroup("Moderator"),
