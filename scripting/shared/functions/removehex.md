---
doc_id: "mta-wiki:8855"
title: "RemoveHex"
source_title: "RemoveHex"
source_url: "https://wiki.multitheftauto.com/wiki/RemoveHex"
revision_id: 78809
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:32.440870+00:00"
---

# RemoveHex

This function is used to remove hexadecimal numbers (expressed as *#XXXXXX*) from strings.

## Syntax

```
string removeHex ( string text [, int digits = math.huge ] )
```

### Required Arguments

- **text**: The text from which you want to remove the hexadecimal numbers.

### Optional Arguments

- **digits**: The number of digits the hexadecimal numbers you want to remove have. If not specified, the function will remove every hexadecimal digit which follows a number sign (#).
...
This code can remove a hex code with a number of characters lower  or equal to the **digits** parameter.
...
The following code can remove hex codes inside a hex code. Ex: #fff#000000fffPlayerName >> PlayerName
...
The following code also removes the hex codes, but if there's a hex code inside a hex code, the outer code will remain. Same result as **Code 1**. Ex: #fff#000000fffPlayerName >> #ffffffPlayerName
...
This example removes colors from player nicknames when they join the server.

```
function removeColorsFromNick()
   local name = getPlayerName(source)
   local new = removeHex(name, 6) -- RGB colors are 6 digits long
   if (new ~= name) then
