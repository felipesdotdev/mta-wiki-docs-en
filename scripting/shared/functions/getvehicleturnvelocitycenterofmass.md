---
doc_id: "mta-wiki:10602"
title: "GetVehicleTurnVelocityCenterOfMass"
source_title: "GetVehicleTurnVelocityCenterOfMass"
source_url: "https://wiki.multitheftauto.com/wiki/GetVehicleTurnVelocityCenterOfMass"
revision_id: 63699
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:29.369378+00:00"
---

# GetVehicleTurnVelocityCenterOfMass

This function gets a vehicle's turn velocity relative to the vehicle's center or mass.

| [[{{{image}}}\|link=\|]] | Note: This useful function requires OOP. |
| --- | --- |
|  |  |
...
```
float,float,float getVehicleTurnVelocityCenterOfMass( vehicle targetVehicle )
```

*Counter Part: [setVehicleTurnVelocityCenterOfMass](mta://scripting/shared/functions/setvehicleturnvelocitycenterofmass.md)*

### Required Arguments

- **targetVehicle**: The vehicle you want to get the turn velocity from.

### Returns

Returns 3 *floats* that represent the vehicle's turning velocity on the x, y and z axis relative to vehicle center of mass or *false* if wrong arguments were passed.
...
addEventHandler("onClientRender",root,function()
	...
	if isElement(vehicle) then
		local tx,ty,tz = getVehicleTurnVelocityCenterOfMass(vehicle)
		if getPedControlState(localPlayer, "steer_forward") then
			tx = tx-0.0005
		end
		if getPedControlState(localPlayer, "steer_back") then
			tx = tx+0.0005
		end
		if getPedControlState(localPlayer, "vehicle_left") then
		...
		setVehicleTurnVelocityCenterOfMass(vehicle,tx,ty,tz)
	end
end)
```

Author: [thisdp](https://wiki.multitheftauto.com/wiki/User:Thisdp)
