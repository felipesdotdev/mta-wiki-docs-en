---
doc_id: "mta-wiki:12037"
title: "Table.element"
source_title: "Table.element"
source_url: "https://wiki.multitheftauto.com/wiki/Table.element"
revision_id: 65233
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:55.484213+00:00"
---

# Table.element

This function allows you to search within a table of any size or dimensions, userdata values.

## Syntax

```
table table.element( table tbl[, string theType=nil ] )
```

### Required Arguments

- **tbl**: The table for search within.

### Optional Arguments

- **theType**: The type of element you want a list of. For example:
...
Returns a new table with only userdata content.
...
local ped = createPed(200, 0, 0, 0)

local myTable = {
	["randomsh*t"] = {
		localPlayer,
		{localPlayer},
		{{localPlayer, ped, "I Luv U", 20, false, true}},
		{["player"] = localPlayer},
		["vehicle"] = vehicle,
		45778972,
		"Hello World",
...
local info = table.element(myTable)

for index, value in ipairs(info) do
	outputChatBox("The type of element is: " .. getElementType(value))
end

-- output:
-- The type of element is: player
-- The type of element is: player
-- The type of element is: player
-- The type of element is: ped
-- The type of element is: player
-- The type of element is: ped
-- The type of element is: vehicle
```
