---
doc_id: "mta-wiki:12692"
title: "Math.getBezierPoint"
source_title: "Math.getBezierPoint"
source_url: "https://wiki.multitheftauto.com/wiki/Math.getBezierPoint"
revision_id: 68405
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:09.219783+00:00"
---
...
```
Vector2/Vector3 math.getBezierPoint( Vector2/Vector3 anchorPoints, float t )
```

### Required Arguments

- **anchorPoints**: The anchor points of bezier curve, curve order is #anchorPoints-1.

- **t**: A float of time of bezier curve.

### Returns

Returns a Vector2/Vector3 indicates the point of the bezier curve at the time of *t*.
...
function math.getBezierPoint(pos,t)
    local retPos = Vector2(0,0)
    local n = #pos-1
    for i=1,n+1 do
        local index = i-1
        retPos = retPos+(1-t)^index*t^(n-index)*pos[i]*C(n,index)
    end
    return retPos
end
```
...
addEventHandler("onClientRender",root,function()
    ...
    for i=0,10 do
        local bz = math.getBezierPoint(anchors,i/10)*10
        dxDrawLine(lastPos,bz,_,2)
        lastPos = bz
    end
end)
```
