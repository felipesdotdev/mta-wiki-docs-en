---
doc_id: "mta-wiki:11137"
title: "IsMouseOnGUICloseButton"
source_title: "IsMouseOnGUICloseButton"
source_url: "https://wiki.multitheftauto.com/wiki/IsMouseOnGUICloseButton"
revision_id: 81822
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:57.207290+00:00"
---

# IsMouseOnGUICloseButton

This function allows you to check whether the mouse cursor/pointer is within the GUI-Window's native close button. This button can only be visible using [guiSetProperty](mta://scripting/client/functions/guisetproperty.md).

 
GUI window with native close button.

## Syntax

```
bool isMouseOnGUICloseButton (element guiElement)
```

### Required arguments

- **guiElement**: The GUI window whose native close button you want to check.

### Return

Returns true if the mouse is over the window's native close button. Returns false if the button is not visible or the mouse is not over it. It also returns false if the *guiElement* is not a gui-window.
...
This example makes the button close the GUI window. (Which it was made for)

```
local sX, sY = guiGetScreenSize ()
theWindow = guiCreateWindow (sX/2 - 100, sY/2 - 50, 200, 100, "Default Window", false)
guiSetProperty (theWindow, "TitlebarEnabled", "False")
guiWindowSetSizable (theWindow, false)
showCursor (true)
theButton = guiCreateButton (50, 30, 100, 40, "Hello", false, theWindow)
...
addEventHandler ("onClientGUIClick", getRootElement(), function (button, state, absoluteX, absoluteY)
    if (state == "up") and (button == "left") then
        if (source == theWindow) then
            if isMouseOnGUICloseButton (theWindow) then
                guiSetVisible (theWindow, false)
                showCursor (false)
            end
        elseif (source == theButton) then
            outputChatBox ("Hello World!", 255, 255, 255)
        end
    end
end)
```
