---
doc_id: "mta-wiki:9183"
title: "StopSoundSlowly"
source_title: "StopSoundSlowly"
source_url: "https://wiki.multitheftauto.com/wiki/StopSoundSlowly"
revision_id: 77134
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:52.713951+00:00"
---

# StopSoundSlowly

This function stops the [sound](mta://reference/misc/sound.md) [element](mta://reference/misc/element.md) slowly.

## Syntax

```
int stopSoundSlowly( element sound )
```

### Required Arguments

- **sound**: The sound element that needs to be stopped.
...
```
function stopSoundSlowly( sound_element )
	if not isElement( sound_element ) then return false end

	local sound_timer_quant = getSoundVolume( sound_element )

	setTimer( function( )
		if not isElement( sound_element ) then return end
		local sound_volume = getSoundVolume( sound_element )
		sound_volume = ( ( sound_volume * 10 ) - ( 0.1 * 10 ) ) / 10
		setSoundVolume( sound_element, sound_volume - 0.1 )
		if sound_volume > 0 then return end
		stopSound( sound_element )
	end, 500, sound_timer_quant * 10 )
end
```
...
```
local sound
addCommandHandler('createSound', function()
	if sound then return end
	sound = playSound('sound.mp3')
end)
addCommandHandler('stopSound', function()
	if not sound then return end
	stopSoundSlowly(sound)
end)
```

Author: J.Chaikos, Galaxy0439, Tracer
