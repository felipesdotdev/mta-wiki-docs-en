---
doc_id: "mta-wiki:9495"
title: "FindRotation3D"
source_title: "FindRotation3D"
source_url: "https://wiki.multitheftauto.com/wiki/FindRotation3D"
revision_id: 76314
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:01.651936+00:00"
---

# FindRotation3D

This function takes two sets of XYZ coordinates. It returns the 3D direction from point A to point B. This can for example be used to make objects "look at" specified point.

## Syntax

```
float float float findRotation3D( float x1, float y1, float z1, float x2, float y2, float z2 )
```
...
- **z1**: The Z coordinate of the starting point.

- **x2**: The X coordinate of the target point.

- **y2**: The Y coordinate of the target point.

- **z2**: The Z coordinate of the target point.
...
This command makes the player's vehicle to rotate at vehicle near it.
*Note: This example needs [OOP](mta://tutorials/oop.md) to be enabled in order to work*

```
local function setRotationAtNearestVehicle()

	if not localPlayer.vehicle then return end

	local nearestVehicle = false
	local dist = 10000
	for index,vehicle in ipairs(getElementsByType("vehicle",root,true)) do
		if getDistanceBetweenPoints3D(localPlayer.vehicle.position,vehicle.position) < dist and localPlayer.vehicle ~= vehicle then
			nearestVehicle = vehicle
			dist = getDistanceBetweenPoints3D(localPlayer.vehicle.position,vehicle.position)
		end
	end

	local x1,y1,z1 = getElementPosition(localPlayer.vehicle)
	local x2,y2,z2 = getElementPosition(nearestVehicle)
	local rx,ry,rz = findRotation3D(x1,y1,z1,x2,y2,z2)

	localPlayer.vehicle.rotation = Vector3(rx,ry,rz)
	
end
	
addCommandHandler("lookatnearestveh",setRotationAtNearestVehicle)
```

Thanks for **ccw** for alternative [getCameraRotation](mta://scripting/client/functions/getcamerarotation.md) function which formula is used in this function. Also thanks for **Doomed_Space_Marine** for more accurate formula for z-axis which is used in [findRotation](mta://scripting/shared/functions/findrotation.md)

Click to collapse [-]
Client-side

This example creates an object on Grove Street that keeps looking at the player.

```
local obj = createObject (2887, 2492, -1667, 16)

addEventHandler("onClientRender", root, function()
    local x, y, z = getElementPosition(localPlayer)
    local xx, yy, zz = getElementPosition(obj)
    local rx, ry, rz = findRotation3D(xx, yy, zz, x, y, z)
    setElementRotation(obj, rx, ry, rz)
end)
```

Example author: [LordHenry](https://wiki.multitheftauto.com/wiki/User:LordHenry)
