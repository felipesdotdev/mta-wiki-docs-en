---
doc_id: "mta-wiki:14622"
title: "GetColorName"
source_title: "GetColorName"
source_url: "https://wiki.multitheftauto.com/wiki/GetColorName"
revision_id: 82437
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:08.904061+00:00"
---

# GetColorName

This function retrieves the nearest color name for a given RGB value using an online API.

| [[{{{image}}}\|link=\|]] | Note: This function requires an active internet connection and fetches the color name from an external API asynchronously. The result is returned via a callback function. |
| --- | --- |
|  |  |
...
```
getColorName( int r, int g, int b, function callback(name) )
```

### Required Arguments

- **r**: Red component (0-255) of the color.

- **g**: Green component (0-255) of the color.

- **b**: Blue component (0-255) of the color.

- **callback**: A function that will receive the color name as a string.

### Returns

- Does not return a value directly.

- Calls the 'callback' function with the color name when the API responds.

- Possible callback values:

```
* 'Error' if the request failed.
 * 'Unknown' if the API did not return a valid name.
 * Color name string, e.g., 'Dark Orange'.
```
...
```
function getColorName(r, g, b, callback)
    local url = string.format("https://www.thecolorapi.com/id?rgb=rgb(%d,%d,%d)", r, g, b)
    fetchRemote(url, function(data, errno)
        if errno == 0 then
            local result = fromJSON(data)
            if result and result.name and result.name.value then
                callback(tostring(result.name.value))
            else
                callback("Unknown")
            end
        else
...
```
getColorName(255, 127, 0, function(name)
    outputChatBox("Color Name: "..name)
end)
```

```
local imangaax = getPlayerFromName("iManGaaX")
local r, g, b = getVehicleColor(getPedOccupiedVehicle(imangaax))
getColorName(r, g, b, function(name)
    outputChatBox("Your Vehicle Color Name: "..name, imangaax)
end)
```

```
-- If a player owns a vehicle and you want to retrieve detailed information about the vehicle, such as its color name
```
