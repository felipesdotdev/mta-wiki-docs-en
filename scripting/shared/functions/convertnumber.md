---
doc_id: "mta-wiki:11893"
title: "FormatNumber"
source_title: "ConvertNumber"
source_url: "https://wiki.multitheftauto.com/wiki/ConvertNumber"
revision_id: 82155
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:35.747386+00:00"
---

# FormatNumber

This function formats large numbers by adding separators for thousands. For example, 100000 becomes 100 000.

## Syntax

```
string formatNumber( int number, [ string sep = "." ] )
```
...
- **sep**: A string used as the thousands separator. Default is **"."**.
...
function formatNumber(n, sep)
    assert(type(n) == "number", "Bad argument @'formatNumber' [Expected number at argument 1, got "..type(n).."]")
    assert(not sep or type(sep) == "string", "Bad argument @'formatNumber' [Expected string or nil at argument 2, got "..type(sep).."]")
    sep = sep or '.'
    local s = string.format("%.0f", n)
    repeat s, c = s:gsub("^(-?%d+)(%d%d%d)", "%1"..sep.."%2") until c == 0
    return s
end
```
...
function convertPlayerMoney()
    ...
    local convertedMoney = formatNumber(pMoney)
