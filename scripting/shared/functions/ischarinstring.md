---
doc_id: "mta-wiki:12155"
title: "IsCharInString"
source_title: "IsCharInString"
source_url: "https://wiki.multitheftauto.com/wiki/IsCharInString"
revision_id: 79994
language: "en"
categories: ["Useful_Functions", "MTA_Wiki:Delete"]
generated_at: "2026-07-26T16:15:53.658672+00:00"
---
...
| --- | --- |
| Reason: This is a wrapper to string.find Actions: Delete (Administrators) - Discuss - What links here - Category |  |

This shared function allows you to check if a char specified is in a string value.

## Syntax

```
bool isCharInString ( string char, string text )
```

### Required arguments

- **char**: The char to find in a string.

- **text**: String value where you want to search *char* value.

### Return

Returns true if *char* is found in specified string, false otherwise.
...
This example checks if char "*" is in a especified string.

```
function checkString ( player, cmd, string )
    if string then
        if isCharInString ( "*", string ) then
            outputChatBox ( "String specified contains * char", player, 0, 255, 0, false )
        else
            outputChatBox ( "String specified does not contain * char.", player, 255, 0, 0, false )
        end
    else
        outputChatBox ( "Enter an string.", player, 255, 0, 0, false )
    end
end
addCommandHandler ( "checkstring", checkString )
```
