---
doc_id: "mta-wiki:12826"
title: "PT-BR/GetResourceSize"
source_title: "GetResourceSize/PT-BR"
source_url: "https://wiki.multitheftauto.com/wiki/GetResourceSize/PT-BR"
revision_id: 69392
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:07:50.138005+00:00"
---

# PT-BR/GetResourceSize

Esta função retorna o tamanho de um resource em **kB**(kilobyte). Os arquivos do resource que não são declarados no [meta](mta://reference/misc/meta.md) não são considerados na contagem.

## Sintaxe

```
float getResourceSize ( string/resource oResource )
```

### Argumentos Necessários

- **oResource**: Uma [string](mta://reference/misc/string.md) contendo o nome do resource ou o objeto [resource](mta://reference/misc/resource.md) (não-elemento) para obter o tamanho.

### Retorna

Retorna um [float](mta://reference/misc/float.md) do tamanho do resource contando com o meta. Senão retorna *false*.
...
function getResourceSize(theResource)
    local linhaAtual = tostring(debug.getinfo(2, 'l').currentline)
    assert(theResource and type(theResource) == 'string' or type(theResource) == 'userdata', "@ 'getResourceSize' (Linha "..linhaAtual..") Esperado um resource ou nome de resource, obtido '"..type(theResource).."'")
    ...
    if not oResource then return false end

    local resourceName = getResourceName(oResource)
    local tamanho = 0
    ...
    if (metaXML) then
        for _, v in ipairs(xmlNodeGetChildren(metaXML)) do
            ...
            if (attribute) then
                local arq = fileOpen(':'..resourceName..'/'..attribute)
                if (arq) then
                    tamanho = tamanho + fileGetSize(arq)
                    fileClose(arq)
                end
            end
        end
        return tonumber(string.format('%.3f', tamanho/1024))
    end
    return false
end
```
...
Este primeiro exemplo cria um comando com a sintaxe **/vertamanho <resource_name>**.

Click to collapse [-]
Lado servidor

```
function resSize(player, cmd, resName)
    if (resName) then
        local tamanho = getResourceSize(tostring(resName))
        if (tamanho) then
            outputChatBox("O tamanho do resource '"..tostring(resName).."' é "..tostring(tamanho).."kB", player)
        else
            outputChatBox('Não foi possível obter um tamanho, verifique os argumentos especificados.', player)
            outputChatBox('Possíveis erros: \nArg#1 não é uma string ou userdata\nResource com o nome especificado não foi encontrado', player)
        end
    else
        outputChatBox('Especifique um nome de resource', player)
    end
end
addCommandHandler('vertamanho', resSize)
```

Este segundo exemplo cria um comando com a sintaxe **/tamanhodeste** que mostra no chat o tamanho do resource em que foi executado o comando.

Click to collapse [-]
Lado servidor

```
function resSize(player, cmd)
    local tamanho = getResourceSize(getThisResource())
    if (tamanho) then
        outputChatBox("O tamanho deste resource("..getResourceName(getThisResource())..") é "..tostring(tamanho).."kB", player)
    else
        outputChatBox('Não foi possível obter um tamanho, verifique os argumentos especificados.', player)
        outputChatBox('Possíveis erros: \nArg#1 não é uma string ou userdata\nResource com o nome especificado não foi encontrado', player)
    end
end
addCommandHandler('tamanhodeste', resSize)
```
