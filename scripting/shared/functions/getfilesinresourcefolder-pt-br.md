---
doc_id: "mta-wiki:12487"
title: "PT-BR/GetFilesInResourceFolder"
source_title: "GetFilesInResourceFolder/PT-BR"
source_url: "https://wiki.multitheftauto.com/wiki/GetFilesInResourceFolder/PT-BR"
revision_id: 72053
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:07:36.544838+00:00"
---

# PT-BR/GetFilesInResourceFolder

Esta função lista todos os arquivos em uma pasta de resource. **OBS**: Esta função deve somente ser executada em server-side.

|  | Nota: A listagem só é feita com arquivos que estão registrado no arquivo meta.xml em atributos de nome src . |
| --- | --- |
|  |  |
...
```
table, table getFilesInResourceFolder ( string directory, [ resource theResource ] )
```

### Argumento Necessário

- **directory**: Uma [string](mta://reference/misc/string-pt-br.md) que especificará a pasta em que deve ser feita a listagem de arquivos. Para obter todos (independente da pasta), use `'/'`
...
- **theResource**: Um [resource](https://wiki.multitheftauto.com/wiki/PT-BR/Recursos)(userdata) que será feita a busca. Caso não especificado, a busca é feita no resource em que a função foi executada.

### Retorna

Retorna duas [tabelas](https://wiki.multitheftauto.com/wiki/PT-BR/Table), a 1° armazenando o nome/caminho dos arquivos obtidos, e a 2° armazenando somente o nome dos arquivos (com suas respectivas extensões). Senão retorna *false*.

###### Formato do valor retornado

- 1° tabela: `{**'Pasta1/Pasta2/arquivo.ext'**, **'Pasta1/arquivo2.ext'**, **'arquivo3.ext'**}`

- 2° tabela: `{**'arquivo.ext'**, **'arquivo2.ext'**, **'arquivo3.ext'**}`
...
function getFilesInResourceFolder(path, res)
	if (triggerServerEvent) then error('A função @getFilesInResourceFolder deve ser usada somente em server-side', 2) end
	
	if not (type(path) == 'string') then
		error("@getFilesInResourceFolder argumento #1. Esperado uma 'string', obtido '"..type(path).."'", 2)
	end
	
	if not (tostring(path):find('/$')) then
		error("@getFilesInResourceFolder argumento #1. O caminho deve conter '/' no final para certificar que seja um diretório.", 2)
	end
	
	res = (res == nil) and getThisResource() or res
	if not (type(res) == 'userdata' and getUserdataType(res) == 'resource-data') then  
		error("@getFilesInResourceFolder argumento #2. Esperado um 'resource-data', obtido '"..(type(res) == 'userdata' and getUserdataType(res) or tostring(res)).."' (tipo: "..type(res)..")", 2)
	end
	...
	local nomeResource = getResourceName(res)
	local Meta = xmlLoadFile(':'..nomeResource..'/meta.xml')
	if not Meta then error("(@getFilesInResourceFolder) Não foi possível obter o 'meta.xml' do resource '"..nomeResource.."'", 2) end
	for _, nod in ipairs(xmlNodeGetChildren(Meta)) do
		...
		if (srcAttribute) then
			...
			local theFile = fileOpen(thisResource and srcAttribute or ':'..nomeResource..'/'..srcAttribute)
			if theFile then
				if (path == '/') then
					...
					filePath = filePath:gsub('/['..charsTypes..']+%.['..charsTypes..']+', '/'):gsub(':'..nomeResource..'/', '')
				...
				outputDebugString("(@getFilesInResourceFolder) Não foi possível verificar o arquivo '"..onlyFileName.."' do resource '"..nomeResource.."'", 2)
			end
		end
	end
	xmlUnloadFile(Meta)
...
Este exemplo cria o comando **/arquivosde <pasta> [resource]** que exibe no chat algumas informações sobre os dados obtidos da busca de arquivos.

```
function verArquivos(player, cmd, folderName, resName)
    if (hasObjectPermissionTo(player, 'function.banPlayer', false)) then -- O jogador só prosseguirá com este comando se tiver permissão de banir
        if (folderName) then -- Se especificou o nome da pasta, então
            local theRes
            if (resName) then -- Se especificou o nome do resource então o definiremos
                theRes = getResourceFromName(tostring(resName))
                if not theRes then -- Se não houver resource com o nome especificado, então avisaremos
                    return outputChatBox("#FF0000##FFFFFF Não existe resource com o nome '"..resName.."'", player, 255, 255, 255, true)
                end
            end
            local files, onlyFilesName = getFilesInResourceFolder(folderName, theRes) -- então faremos a busca
            if (files and onlyFilesName) then -- Se deu tudo certo, então mostraremos os dados de resposta
                outputChatBox('===== Arquivos obtidos! =====', player, 255, 255, 255, true)
                outputChatBox('=> Pasta: '..tostring(folderName), player, 255, 255, 255, true)
                outputChatBox('=> Resource: '..tostring(resName), player, 255, 255, 255, true)
                outputChatBox('=> Total de arquivos: '..tostring(#files)..' arquivos', player, 255, 255, 255, true)
                outputChatBox('=> Arquivos: #00FF00'..tostring(table.concat(onlyFilesName, ', ')), player, 255, 255, 255, true)
                outputChatBox('==============================', player, 255, 255, 255, true)
            else
                outputChatBox('Não foi possível obter a lista de arquivos', player)
            end
        else
            outputChatBox('Especifique o nome da pasta que você quer procurar os arquivos', player)
        end
    else
        outputChatBox('# Você não tem permissão para executar este comando.', player, 255, 0, 0, false)
    end
end
addCommandHandler('arquivosde', verArquivos)
```
...
Este exemplo cria o comando **/arquivosaqui <pasta>** que exibe no chat informações sobre os arquivos obtidos pela função útil.

```
function verArquivos(player, cmd, folderName)
    if (hasObjectPermissionTo(player, 'function.banPlayer', false)) then -- Somente prosseguirá com este comando quem tiver permissão para banir jogadores
        if (folderName) then -- Se especificou o nome da pasta que quer buscar, então...
            local files, onlyFilesName = getFilesInResourceFolder(folderName)
            if (files and onlyFilesName) then -- Se obtivemos dados sobre os arquivos, então exibiremos
                outputChatBox('===== Arquivos obtidos! =====', player, 255, 255, 255, true)
                outputChatBox('=> Pasta: '..tostring(folderName), player, 255, 255, 255, true)
                outputChatBox('=> Resource: '..getResourceName(getThisResource()), player, 255, 255, 255, true)
                outputChatBox('=> Total de arquivos: '..tostring(#files)..' arquivos', player, 255, 255, 255, true)
                outputChatBox('=> Arquivos: #00FF00'..tostring(table.concat(onlyFilesName, ', ')), player, 255, 255, 255, true)
                outputChatBox('==============================', player, 255, 255, 255, true)
            else
                outputChatBox('#FF0000##FFFFFF Não foi possível obter os arquivos na pasta especificada', player, 255, 255, 255, true)
            end
        else
            outputChatBox('#FF0000##FFFFFF Especifique o nome da pasta que queira buscar', player, 255, 255, 255, true)
        end
    else
        outputChatBox('# Você não tem permissão para executar este comando', player, 255, 0, 0, false)
    end
end
addCommandHandler('arquivosaqui', verArquivos)
```

## Créditos

- **Autor:** [KronoS Lettify](https://wiki.multitheftauto.com/wiki/User:Lettify).
