---
doc_id: "mta-wiki:14178"
title: "DxFade"
source_title: "DxFade"
source_url: "https://wiki.multitheftauto.com/wiki/DxFade"
revision_id: 78230
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:14:48.637417+00:00"
---

# DxFade

This function allows you to fade-in or fade-out any dxDraw by gradually changing its alpha value.

| [[{{{image}}}\|link=\|]] | Note: You can fade up to 3 values at the same time, as long as they are properly declared together on the same interpolateBetween function and on the dxFade function parameters. |
| --- | --- |
|  |  |

| [[{{{image}}}\|link=\|]] | Note: If you need to start another fade while this function is already fading something, you will need another dxFade function. |
| --- | --- |
|  |  |
...
```
bool dxFade (bool showing, int maxAlpha, float theDuration, string strEasingType, [float period, float amplitude, float overshoot])
```

### Required Arguments

- **showing:** A boolean to fade-in (true) or fade-out (false).

- **maxAlpha:** The amount of [alpha](http://en.wikipedia.org/wiki/RGBA_color_space) the DX will use while visible (1-255).

- **theDuration:** The number of milliseconds that the fade will elapse (1000 milliseconds = 1 second).

- **strEasingType:** The [easing function](mta://reference/misc/easing.md) to use for the interpolation.

### Optional Arguments

- **period:** The period of the [easing function](mta://reference/misc/easing.md) (only some easing functions use this parameter).

- **amplitude:** The amplitude of the [easing function](mta://reference/misc/easing.md) (only some easing functions use this parameter).

- **overshoot:** The overshoot of the [easing function](mta://reference/misc/easing.md) (only some easing functions use this parameter).
...
```
function dxFade(showing, maxAlpha, theDuration, strEasingType, period, amplitude, overshoot)
    ...
    if showing then
        ...
        function renderIn()
            ...
            currentAlpha = interpolateBetween (0, 0, 0, maxAlpha, 0, 0, progress, strEasingType, period, amplitude, overshoot)
        ...
        function renderOut()
            ...
            local duration = endTime - start
            local progress = elapsedTime / duration
            currentAlpha = interpolateBetween (maxAlpha, 0, 0, 0, 0, 0, progress, strEasingType, period, amplitude, overshoot)
...
This example shows an image on the screen that fade-in during 1 second and after 3 seconds, it fade-out during 1 second.

```
local screen = Vector2 (guiGetScreenSize())

function drawSomething()
    dxDrawImage (0, 0, screen.x, screen.y, "image.png", 0, 0, 0, tocolor (255, 255, 255, currentAlpha or 0)) -- You need to provide the image.
end
addEventHandler ("onClientRender", root, drawSomething)

addEventHandler ("onClientResourceStart", resourceRoot, function ()
    dxFade (true, 255, 1000, "Linear")
    
    setTimer (function()
        dxFade (false, 255, 1000, "Linear")
    end, 3000, 1)
end)
```
