---
doc_id: "mta-wiki:13206"
title: "AddTableChangeHandler"
source_title: "AddTableChangeHandler"
source_url: "https://wiki.multitheftauto.com/wiki/AddTableChangeHandler"
revision_id: 79059
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:20.911669+00:00"
---

# AddTableChangeHandler

| [[{{{image}}}\|link=\|]] | Important Note: do not use this function for local variables |
| --- | --- |
|  |  |
...
```
bool addTableChangeHandler(string tablename, function func)
```

### Arguments

- **tablename**: The variable [table](mta://reference/misc/table.md) name.

- **func**: The function.

### Returns

The func returns 5 arguments the tablename, the table, the key, the old value and the new value.
...
```
addTableChangeHandler = function(t, f)
	local f = f
...
```
function addTableChangeHandler(tablename, func)

    local tablename, func = tablename, func
    local parts = {}
    local name
    local t = _G
    local i = 0

    for word in tablename:gmatch("[^%.]+") do
        i = i + 1
        parts[i] = word
    end

    for i = 1, #parts-1 do
        name = not name and parts[i] or name..'.'..parts[i]

        if t[parts[i]] then
            t = t[parts[i]]
        else
            return error('The table ["'..name..'"] does not exist' )
        end
     end

    local last = parts[#parts]
    if not t[last] then return error('The table ["'..(name..'.'..last)..'"] does not exist') end

    local old = t[last]
    t[last] = setmetatable({}, {
        __index = old,
        __newindex = function(self, key, value)
            local oldValue = rawget(old, key)
            rawset(old, key, value)
            if func then
               func(tablename, old, key, oldValue, value)
            end
         end,
    })
    return t[last]
...
addCommandHandler('smoney',
	function()
		Config.player.money = math.random(999)
	end
)

addTableChangeHandler('Config.player',
	function(name, table, key, old, new)
		print(('tablename: %s | key: %s | Old: %s | New: %s'):format(tostring(name), tostring(key), tostring(old), tostring(new)))
	end
)

Config.player.money = 1
Config.player.money = 50
```
