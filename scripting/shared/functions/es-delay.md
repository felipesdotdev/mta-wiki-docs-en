---
doc_id: "mta-wiki:12817"
title: "ES/Delay"
source_title: "Es/Delay"
source_url: "https://wiki.multitheftauto.com/wiki/Es/Delay"
revision_id: 69208
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:14:59.668104+00:00"
---

# ES/Delay

Esta Funcion nos sirve para dar un retraso entre una linea de codigo y la(s) siguiente(s)

## Sintaxis

```
bool delay(func, time, ...)
```

### Argumentos Requeridos

- **func**: Debe ir la funcion en donde se situara el delay.

- **time**: El tiempo que durara el delay.

- **( ... )**: Los argumentos que se guardaran de la funcion.

## Notas

PD : Si Piensa utilizar esto debe agregar un delay minimo al inicio del todo.

PD 2: Suele mandar un warning a la primera ejecucion pero es normal, no causa problemas.
...
function delay(func, time, ...)
	local arg = {...}
	if not co[tostring(func)] then
		co[tostring(func)] = coroutine.create(func)
		setTimer(function()
			coroutine.resume(co[tostring(func)], unpack(arg))
		end,time,1)
		return
	end
	setTimer(function()
...
```
--[[Si utilizaremos delay en alguna funcion antes que nada al inicio 
de esta tiene q haber un delay de 50 para que esta se configure de igual forma teneis
que meter los argumentos a utilizar en el primer delay para que esta se le guarden los datos: por ejemplo]]
function cuenta_regresiva(player)
	player = (player.type ~= 'player' and Player.getRandom()) or player
	delay(cuenta_regresiva, 50, player) 
	coroutine.yield()

	player:outputChat('3...')

	delay(cuenta_regresiva, 1000) 
	coroutine.yield()
...
addEventHandler( "onResourceStart", getRootElement(), cuenta_regresiva)
```
