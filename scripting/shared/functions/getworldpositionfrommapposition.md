---
doc_id: "mta-wiki:13194"
title: "GetWorldPositionFromMapPosition"
source_title: "GetWorldPositionFromMapPosition"
source_url: "https://wiki.multitheftauto.com/wiki/GetWorldPositionFromMapPosition"
revision_id: 76262
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:30.504036+00:00"
---

# GetWorldPositionFromMapPosition

Map being draw on the screen

This function allows you to get the world position from a clicked F11 map position.

## Syntax

```
float, float, float getWorldPositionFromMapPosition (float cursorX, float cursorY, bool relative)
```

### Required arguments

- **cursorX**: A float representing the cursor X position.

- **cursorY**: A float representing the cursor Y position.

- **relative**: A bool that indicates if the cursor x/y positions are relative to the screen or absolute.

### Return

Returns three *float*s indicating the world position of the clicked map position, *x*, *y* and *z* respectively. Returns *false* if the clicked position is out of the map being draw on the screen.
...
function getWorldPositionFromMapPosition (cursorX, cursorY, relative)
    if (relative) then
        local screen = Vector2(guiGetScreenSize()) -- Get screen resolution.
        cursorX, cursorY = cursorX * screen.x, cursorY * screen.y -- Converts cursor relative position to absolute position.
    end
    local mx, my, Mx, My = getPlayerMapBoundingBox() -- Get map start and finish positions,
    local mapMin = Vector2(mx, my) -- Separate map start position in a Vector2 variable.
    local mapMax = Vector2(Mx, My) -- Separata map finish position in a Vector2 variable.
    if (cursorX >= mapMin.x and cursorY >= mapMin.y and cursorX <= mapMax.x and cursorY <= mapMax.y) then -- If the user clicked inside the map area, then:
        local relPos = Vector2((cursorX - mapMin.x) / (mapMax.x - mapMin.x), (cursorY - mapMin.y) / (mapMax.y - mapMin.y)) -- Make cursor position relative to map position and scale.
        local worldPlanePos = Vector2(6000 * (relPos.x - 0.5), 3000 - (relPos.y * 6000)) -- Converts relative map position to world position.
        return worldPlanePos.x, worldPlanePos.y, getGroundPosition(worldPlanePos.x, worldPlanePos.y, 530) -- Returns the world position. Also adds a Z value to the position, it becomes 0 if the position is too far away from localPlayer.
    end
    return false -- Returns false if the clicked position is out of the map.
end
```

**Author:** [LordHenry](https://wiki.multitheftauto.com/wiki/User:LordHenry)
...
This example creates a blip on the radar and F11 map where the player clicked. The player must use /map command instead of F11 to show the map with cursor.

```
local theBlip = false

function mapClick (cursorPosX, cursorPosY)
    if (getWorldPositionFromMapPosition (cursorPosX, cursorPosY, true)) then -- If the user clicked inside the map area, then:
...
addCommandHandler ("map", toggleCustomMap) -- Show F11 map, cursor and enables the blip creation buttons.

function mapControls (button, pressing)
    if (pressing) then -- Avoid double triggering.
        if (button == "mouse1") then -- If left mouse button is used, create/remove the blip from the map.
            local cursorPos = Vector2 (getCursorPosition()) -- Get cursor relative position.
            mapClick (cursorPos.x, cursorPos.y)
        elseif (button == "mouse2") then -- If right mouse button is used, hide F11 map and disable blip creation buttons.
            toggleCustomMap()
        elseif (button == getKeyBoundToCommand ("radar")) then -- If F11 map is visible using /map command, disable F11 so the player must use right mouse button to hide it.
            cancelEvent()
        end
    end
end
