---
doc_id: "mta-wiki:14237"
title: "GetBanBySerial"
source_title: "GetBanBySerial"
source_url: "https://wiki.multitheftauto.com/wiki/GetBanBySerial"
revision_id: 78865
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:07.188396+00:00"
---

# GetBanBySerial

This function returns the ban if the serial is banned, otherwise false.

## Syntax

```
ban getBanBySerial ( string serial )
```

### Required Arguments

- **serial**: The banned player serial.
...
This example removes the player's ban from the serial provided.

```
addCommandHandler("unban", function(source, _, serial)
    if not serial then
        outputChatBox("The command is /unban serial", source, 255, 0, 0)
        return
    end
    local ban = getBanBySerial(serial)
    if ban then
        removeBan(ban, source)
        outputChatBox("The serial "..serial.." has been unbanned!", source, 0, 255, 0)
    else
        outputChatBox("This serial is not banned!", source, 255, 0, 0)
    end
end)
```
