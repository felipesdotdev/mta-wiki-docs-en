---
doc_id: "mta-wiki:14082"
title: "SetVehicleWheelModel"
source_title: "SetVehicleWheelModel"
source_url: "https://wiki.multitheftauto.com/wiki/SetVehicleWheelModel"
revision_id: 77158
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:49.182007+00:00"
---

# SetVehicleWheelModel

This function changes the wheel model of the informed vehicle.

## Syntax

```
int setVehicleWheelModel(element veh, int wheelId)
```

## Required arguments

- **veh**: The vehicle you want to change the wheel.

- **wheelId**: The ID of the new wheel you want.

## Returns

Returns the ID of the new wheel if successfully changed, otherwise it returns false.

## Code

Click to collapse [-]
Server/Clientside script

```
validWheels = {[1025] = true, [1073] = true, [1074] = true, [1075] = true, [1076] = true, [1077] = true, [1078] = true, [1079] = true, [1080] = true, [1081] = true, [1082] = true, [1083] = true, [1084] = true, [1085] = true, [1096] = true, [1097] = true, [1098] = true}

function setVehicleWheelModel(veh, wheelId)
    if veh and isElement(veh) then
        if validWheels[tonumber(wheelId)] then
            local success = addVehicleUpgrade(veh, tonumber(wheelId))
            if success then
                return wheelId
            end
        end
...
```
addCommandHandler("wset", function(player, _, wheelId)
    if wheelId and tonumber(wheelId) then
        local veh = getPedOccupiedVehicle(player)
        if veh and isElement(veh) then
            local result = setVehicleWheelModel(veh, wheelId)
            if result then
                outputChatBox("Wheel id is now "..result, player, 0, 255, 0)
            else
