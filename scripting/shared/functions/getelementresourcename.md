---
doc_id: "mta-wiki:14584"
title: "GetElementResourceName"
source_title: "GetElementResourceName"
source_url: "https://wiki.multitheftauto.com/wiki/GetElementResourceName"
revision_id: 82249
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:11.982542+00:00"
---

# GetElementResourceName

This function returns the name of the resource that created an element.

## Syntax

```
string getElementResourceName ( element theElement )
```

### Required arguments

- **theElement**: the [element](mta://reference/misc/element.md) you want to get the resource name of.

## Returns

This function always returns a *string* containing the [element](mta://reference/misc/element.md)'s creator or a string containing information on how it failed to do so.
...
function getElementResourceName(element)
    ...
    local maxHops = 50
    local path = getElementType(element)
    local chain = { element }

    -- Build parent chain
    local current = element
    while current and maxHops > 0 do
        current = getElementParent(current)
        if current then
            table.insert(chain, current)
            path = path .. " -> " .. getElementType(current)
        end
        maxHops = maxHops - 1
    end

    -- Try to match one of the parents to a resource root
    for _, res in ipairs(getResources()) do
        local resRoot = getResourceRootElement(res)
        for _, ancestor in ipairs(chain) do
            if ancestor == resRoot then
                return getResourceName(res)
            end
        end
    end

    return "No matching resource found (path: " .. path .. ")"
end
```
