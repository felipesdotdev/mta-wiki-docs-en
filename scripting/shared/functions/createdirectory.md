---
doc_id: "mta-wiki:14546"
title: "CreateDirectory"
source_title: "CreateDirectory"
source_url: "https://wiki.multitheftauto.com/wiki/CreateDirectory"
revision_id: 81920
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:37.410718+00:00"
---

# CreateDirectory

This function creates a directory at the specified path within the resource's file system. In MTASA, directories are implicitly created by creating a file within them, and this function handles that process automatically by creating and then removing a temporary file.
...
- **directoryPath:** A string representing the path of the directory to create, relative to the resource's root folder (e.g., "myfolder" or "myfolder/subfolder").

### Returns

Returns *true* if the directory was successfully created, *false* if the creation failed or if invalid arguments were passed.
...
This function is a simple and efficient way to create directories in MTASA. It uses the MTASA file system’s behavior of implicitly creating directories when a file is created within them. A temporary file is created and then deleted to leave the directory empty and ready for use.

## Usage

This example shows how to call the `createDirectory` function to create a directory and provide feedback in the server console.

```
-- Create a directory called "logs" in the resource folder
local success = createDirectory("logs")
if success then
    outputConsole("Directory 'logs' was created successfully!")
else
    outputConsole("Failed to create directory 'logs'.")
end

-- Create a nested directory structure
local nestedSuccess = createDirectory("data/user/files")
if nestedSuccess then
    outputConsole("Nested directory 'data/user/files' was created successfully!")
else
    outputConsole("Failed to create nested directory 'data/user/files'.")
end
```

## Notes

- This function must be written in a server-side script because file system operations like `fileCreate`, `fileWrite`, `fileClose`, and `fileDelete` are restricted to server-side in MTASA for security and synchronization reasons.

- The directory path is relative to the resource's root folder (e.g., `:resourceName/directoryPath/`).

- Ensure the resource has appropriate file permissions in the `meta.xml` file:
