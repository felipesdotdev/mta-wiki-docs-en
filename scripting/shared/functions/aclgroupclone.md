---
doc_id: "mta-wiki:9260"
title: "AclGroupClone"
source_title: "AclGroupClone"
source_url: "https://wiki.multitheftauto.com/wiki/AclGroupClone"
revision_id: 78382
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:12:15.200631+00:00"
---
...
function aclGroupClone(clonedGroup, groupName, aclsClone, objectsClone)
    assert(type(clonedGroup) == "string", "Bad argument @ 'aclGroupClone' [Expected string at argument 1, got "..type(clonedGroup).."]")
    assert(type(groupName) == "string", "Bad argument @ 'aclGroupClone' [Expected string at argument 2, got "..type(groupName).."]")
    assert(type(aclsClone) == "boolean", "Bad argument @ 'aclGroupClone' [Expected boolean at argument 3, got "..type(aclsClone).."]")
    assert(type(objectsClone) == "boolean", "Bad argument @ 'aclGroupClone' [Expected boolean at argument 4, got "..type(objectsClone).."]")

    local cloned = aclGetGroup(clonedGroup)
    local newGroup = aclCreateGroup(groupName)

    assert(cloned, "Bad argument @ 'aclGroupClone' [Expected acl-group at argument 1, not found group with name '"..clonedGroup.."']")
    assert(newGroup, "Bad argument @ 'aclGroupClone' [Expected acl-group at argument 2, there is another group with name '"..groupName.."']")
    ...
    if (objectsClone) then
        for index, value in ipairs(aclGroupListObjects(cloned)) do
            aclGroupAddObject(newGroup, value)
        end
    end

    outputDebugString("'aclGroupClone' [Group '"..clonedGroup.."' has been cloned Successfully to '"..groupName.."']", 3)
    return true
end
```

**Edited by:** [Youssef Maged (iManGaaX)](https://wiki.multitheftauto.com/wiki/User:IManGaaX)
