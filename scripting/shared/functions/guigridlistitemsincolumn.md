---
doc_id: "mta-wiki:10256"
title: "ES/GuiGridListItemsInColumn"
source_title: "GuiGridListItemsInColumn"
source_url: "https://wiki.multitheftauto.com/wiki/GuiGridListItemsInColumn"
revision_id: 62465
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:37.997506+00:00"
---

# ES/GuiGridListItemsInColumn

Esta Función verifica si todos los rows contienen items en una columna especifica de una [guiCreateGridList](mta://scripting/client/functions/guicreategridlist.md)

## Sintaxis

```
bool guiGridListItemsInColumn(list, column)
```

### Argumentos Requeridos

- **list**: Una lista creada con [guiCreateGridList](mta://scripting/client/functions/guicreategridlist.md).

- **column**: La index de la columna a la cual deseas verificar.

### Returns

Si colocaste bien los parámetros retorna un valor verdadero, caso contrario falso.
...
```
function guiGridListItemsInColumn(list,column)
	local string_temporal = ''
	local rows = guiGridListGetRowCount( list )
	for index=0,tonumber(rows)-1 do
		local items = guiGridListGetItemText( list, index, column )
		if items ~= "" then
			string_temporal = string_temporal..'True, '
		else
			string_temporal = string_temporal..'False, '
		end
	end
	if not string_temporal:find('False') then
		return true
	end
	return false
end
