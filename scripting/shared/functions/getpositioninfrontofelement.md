---
doc_id: "mta-wiki:14188"
title: "GetPositionInFrontOfElement"
source_title: "GetPositionInFrontOfElement"
source_url: "https://wiki.multitheftauto.com/wiki/GetPositionInFrontOfElement"
revision_id: 81948
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:22.109544+00:00"
---

# GetPositionInFrontOfElement

This function returns position in provided distance away from element, including element's rotation

## Syntax

```
float, float, float getPositionInFrontOfElement( element theElement, float distance )
```

### Required Arguments

- **theElement**: The element you want to get position in front of.

- **distance**: Distance from an element you want to get position in.

### Returns

Returns three *floats* indicating the position in front of the element, *x*, *y* and *z* respectively.
...
function getPositionInFrontOfElement(theElement, distance)
	...
	local _, _, rz = getElementRotation(theElement)
...
```
addCommandHandler("createtreeonmyface", function(_, distance)
    local x, y, z = getPositionInFrontOfElement(localPlayer, tonumber(distance))
    if (x) then 
        createObject(660, x, y, z)
    end 
end)
```
...
```
addCommandHandler("createtreeonmyface", function(player, _, distance)
    local x, y, z = getPositionInFrontOfElement(player, tonumber(distance))
    if (x) then 
        createObject(660, x, y, z)
    end 
end)
