---
doc_id: "mta-wiki:11176"
title: "IsValueInTable"
source_title: "IsValueInTable"
source_url: "https://wiki.multitheftauto.com/wiki/IsValueInTable"
revision_id: 64886
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:01.649486+00:00"
---
...
```
bool isValueInTable ( table theTable, value, int columnID )
```

### Required arguments

- **theTable**: the [table](mta://reference/misc/table.md) to check if value exist.

- **value**: the value to check.

- **columnID**: The column number within the [table](mta://reference/misc/table.md).

## Returns

This function returns a boolean value, true if the value exists in the table, false if the value does not exist in the table and ID of row to value in table.
...
This example verifies if the written word does not exist within the table and then adds it to the table, otherwise a warning will be issued.
...
function greetingCheck(CMD,value)
local value = value or nil
local checker,id = isValueInTable(greetingTable,value,1)
    if not checker then
        table.insert(greetingTable,{value})
        outputChatBox('('..value..')['..#greetingTable..'] added to table.')
    else
    outputChatBox('('..value..')['..id..'] already exist in the table.')
    end
end
addCommandHandler('addgreeting',greetingCheck)
```
