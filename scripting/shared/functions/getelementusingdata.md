---
doc_id: "mta-wiki:13192"
title: "GetElementUsingData"
source_title: "GetElementUsingData"
source_url: "https://wiki.multitheftauto.com/wiki/GetElementUsingData"
revision_id: 78397
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:12.435241+00:00"
---

# GetElementUsingData

This function returns table elements that has [element data](mta://scripting/shared/functions/setelementdata.md).

## Syntax

```
table getElementUsingData ( string key, var value, [ string type ] )
```

## Required arguments

- **key**: The key that the wanted element has.

- **value**: The value that the wanted element has.

## Additional arguments

- **type**: The [element type](mta://reference/misc/element.md) that the wanted element has.

## Returns

Returns table elements that contains the elements data with the given key and value.
...
```
function getElementUsingData(key, value, type)
    type = type or "player"
    local returnedArray = {}
    for indexTable, elementTable in ipairs(getElementsByType(type)) do
        local elementData = getElementData(elementTable, key)
        if elementData == value then
            returnedArray[#returnedArray + 1] = elementTable
        end
    end
