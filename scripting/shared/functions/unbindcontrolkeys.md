---
doc_id: "mta-wiki:8070"
title: "BindControlKeys"
source_title: "UnbindControlKeys"
source_url: "https://wiki.multitheftauto.com/wiki/UnbindControlKeys"
revision_id: 42848
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:17:01.471994+00:00"
---

# BindControlKeys

These useful clientside functions bind the keys bound to a control individually, using [bindKey](mta://scripting/shared/functions/bindkey.md), and unbind them using [unbindKey](mta://scripting/shared/functions/unbindkey.md). The objective of these functions is to bypass the limitation of binding a control directly which ignores key presses if the control is deactivated at the moment (so the script would be able to react if the player presses *aim_weapon* in a vehicle, for example).
...
- **control:** The name of the control to bind or unbind. See [control names](mta://reference/misc/control-names.md).

- **bindData:** (*bindControlKeys* only) Configures how the control keys will be bound. See clientside [bindKey](mta://scripting/shared/functions/bindkey.md) syntaxes for reference.
...
**You can only have one control keys bind per control**. That should be enough for most scripts. If you want to create more binds, copy these functions into a new script file (or [resource](mta://reference/misc/resource.md)) and mark them as *local*.
...
This example allows players to toggle driveby by pressing or releasing the *aim_weapon* control.
...
- [unbindControlKeys](mta://scripting/shared/functions/unbindcontrolkeys.md) » This function allows you to unbind each key bound to a control individually. Use this function with bindControlKeys.
