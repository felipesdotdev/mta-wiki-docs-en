---
doc_id: "mta-wiki:14274"
title: "GregorianToJalali"
source_title: "GregorianToJalali"
source_url: "https://wiki.multitheftauto.com/wiki/GregorianToJalali"
revision_id: 79070
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:31.385953+00:00"
---

# GregorianToJalali

This function converts Gregorian date to Jalali (Shamsi) date

## Syntax

```
table gregorianToJalali(year, month, day)
```
...
Returns a table of substrings with different time format.

| Member | Meaning | Range |
| --- | --- | --- |
| day | day of the month in jalali/shamsi date (1-31) | 1-31 |
| month | month in jalali/shamsi date (1-12) | 1-12 |
| year | year in jalali/shamsi date |  |

## Code

Click to collapse [-]
Function source (Server/Client)

```
function gregorianToJalali(year, month, day)
    local result = {}
    local array = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334}
    ...
    local days = math.floor((temp + 3) / 4) + (365 * year) - math.floor((temp + 99) / 100) - 80 +
        array[month] + math.floor((temp + 399) / 400) + day

    result.year = result.year + 33 * math.floor(days / 12053)
    days = days % 12053
    result.year = result.year + 4 * math.floor(days / 1461)
    days = days % 1461

    if days > 365 then
        result.year = result.year + math.floor((days - 1) / 365)
        days = (days - 1) % 365
    end
...
```
local jalali = gregorianToJalali(2024, 4, 14)
print( string.format('%04d/%02d/%02d', jalali.year, jalali.month, jalali.day) ) -- 1403/01/26
```
