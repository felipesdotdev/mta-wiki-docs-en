---
doc_id: "mta-wiki:12481"
title: "GetNearestElement"
source_title: "GetNearestElement"
source_url: "https://wiki.multitheftauto.com/wiki/GetNearestElement"
revision_id: 80815
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:15.874902+00:00"
---

# GetNearestElement

This function returns the nearest [element](mta://reference/misc/element.md) (of a specific type) to a [player](mta://reference/misc/player.md).
...
- **player**: The player who you want to get the nearest element from.

- **type**: The type of the element you want to get. ("player", "ped", "object", etc.)

- **distance**: A number for the maximum distance the element can be from the player.

### Returns

Returns an [element](mta://reference/misc/element.md) if it's within the specified **distance**, *false* otherwise.
...
This example outputs the name of the nearest vehicle within 300 metres.

Click to collapse [-]
Client

```
addCommandHandler("findVehicle", function()
    local nearestVehicle = getNearestElement(localPlayer, "vehicle", 300) -- nearest vehicle within 300m
    if nearestVehicle then -- if there is a vehicle (true)
        outputChatBox("#7aff69"..getVehicleName(nearestVehicle).."#ffffff is the nearest vehicle!",255,255,255,true)
    else -- no vehicle (false)
        outputChatBox("There are#ff2e2e NO#ffffff vehicles nearby",255,255,255,true)
    end
end)
```

**Modified version of** [getNearestVehicle](mta://scripting/shared/functions/getnearestvehicle.md)
