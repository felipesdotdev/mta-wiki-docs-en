---
doc_id: "mta-wiki:13907"
title: "GetElementRelatedAngle"
source_title: "GetElementRelatedAngle"
source_url: "https://wiki.multitheftauto.com/wiki/GetElementRelatedAngle"
revision_id: 76387
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:11.831560+00:00"
---

# GetElementRelatedAngle

This function returns the related angle between one [element](mta://reference/misc/element.md) to another. This is useful to check if a player is on the left side of a vehicle, for example.

## Syntax

```
float getElementRelatedAngle (element thisElement, element targetElement)
```

### Required Arguments

- **thisElement**: The element whose **XY position** will be checked to obtain the angle relative to the other element.

- **targetElement**: The element whose **XY position** and **Z rotation** will be checked to obtain the angle relative to the other element.

### Returns

Returns a float indicating the related angle between the first element and the second. It returns *false* if invalid parameters are provided.
...
```
function getElementRelatedAngle (thisElement, targetElement)
    if isElement(thisElement) and isElement(targetElement) then
        local pointX, pointY = getElementPosition(thisElement)
        local x, y = getElementPosition(targetElement)
        local _, _, rz = getElementRotation(targetElement)
        local anglePointToElement = math.atan2(pointX - x, pointY - y)
        local formattedAnglePointToElement = math.deg(anglePointToElement)
        local elementRelatedPosition = formattedAnglePointToElement + rz
        
        -- Valid angles must be from 0 to 359.9999999999
        if (elementRelatedPosition < 0) then
            elementRelatedPosition = elementRelatedPosition + 360 -- Sets any negative angle to valid angle.
        elseif (elementRelatedPosition >= 360) then
            elementRelatedPosition = elementRelatedPosition - 360 -- Sets any angle higher than 360 to valid angle. 360 becomes 0.
        end
        
        return elementRelatedPosition
    else
        return false
    end
end
...
This example allows the player to press *H* near a vehicle to open its door according to the vehicle related angle. If the player is on the front of the vehicle, it opens the hood, if the player is on the back of the vehicle, it opens the trunk and so on.

```
function getNearestElement(thePlayer, elementType) -- Useful function (simplified version)
    local dist = 20 -- Only gets elements nearer than this distance.
    local x, y, z = getElementPosition(thePlayer)
    local nearest = false
    for _,veh in pairs(getElementsByType(elementType)) do
        local vx, vy, vz = getElementPosition(veh)
        local newDist = getDistanceBetweenPoints3D(x, y, z, vx, vy, vz)
...
bindKey("h", "down", function(key, state)
    local veh, distance = getNearestElement(localPlayer, "vehicle")
    if veh and isElement(veh) then
        if distance < 5 then
            local angle = getElementRelatedAngle(localPlayer, veh)
            local door
            if angle >= 340 or angle < 20 then
                door = 0
            elseif angle >= 20 and angle < 90 then
                door = 3
            elseif angle >= 90 and angle < 160 then
                door = 5
            elseif angle >= 160 and angle < 200 then
                door = 1
            elseif angle >= 200 and angle < 270 then
                door = 4
            elseif angle >= 270 and angle < 340 then
                door = 2
            end
            if getVehicleDoorOpenRatio (veh, door) > 0 then
                setVehicleDoorOpenRatio (veh, door, 0, 500)
            else
                setVehicleDoorOpenRatio (veh, door, 1, 500)
            end
        end
    end
end)
