---
doc_id: "mta-wiki:14160"
title: "Debounce"
source_title: "Debounce"
source_url: "https://wiki.multitheftauto.com/wiki/Debounce"
revision_id: 77915
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:11:20.430016+00:00"
---

# Debounce

This function  is removing unwanted input noise from buttons, switches or other user input.
see: [https://www.techtarget.com/whatis/definition/debouncing](https://www.techtarget.com/whatis/definition/debouncing)

## Syntax

```
void debounce( number timeout, function callback, ...arguments)
```
...
function debounce(timeout, callback, ...)
    ...
    if(isTimer(debounces[id]))then
        killTimer(debounces[id])
        debounces[id] = nil;
    end
    debounces[id] = setTimer(function(...)
        callback(...) 
        debounces[id] = nil;
    end, timeout, 1, ...)
end
...
This example will ignore first 9 calls and only last one get called within 500ms timeout
...
for i=1,10 do
    debounce(500, callback, i);
end

-- Result in console: 10
```
