---
doc_id: "mta-wiki:14239"
title: "FixPersianString"
source_title: "FixPersianString"
source_url: "https://wiki.multitheftauto.com/wiki/FixPersianString"
revision_id: 82482
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:01.959621+00:00"
---
...
```
string fixPersianString ( string text )
```
...
- Added new functionality to support hex color codes.

- Performance improvements.

- Support Multiline Strings

- Bug fixes: The problem of using multiple English sentences in the text and reversing the position of words has been resolved. The problem of using numbers and reversing numbers has been resolved.

## Code

*NOTE:* This function requires function **[Check](mta://scripting/shared/functions/check.md)** in order to work correctly.
...
function fixPersianString(str)
    assert(type(str) == 'string', "Expected a string, got " .. type(str))
    local unicode = '‮'  
    if not str:match("[\216-\219][\128-\191]") then
        return str
    end
    str = str:gsub("\r\n", "\r\n" .. unicode)
    str = str:gsub("\n", "\n" .. unicode)
    if str:find("#%x%x%x%x%x%x") then
        ...
        for line in (str .. "\n"):gmatch("(.-)\n") do
            ...
            for word in line:gmatch("%S+") do
                word = word:gsub("%d+", function(d) return d:reverse() end)
        ...
        str = unicode .. str:gsub("%f[%w]%w+%f[%W]", function(w)
            w = w:gsub("%d+", function(d) return d:reverse() end)
            return w:reverse()
        end)
    end
    str = str:gsub("%f[%a]([%a%s]+)%f[%A]", function(block)
        local tokens = {}
        local wordIndices = {}

        for token in block:gmatch("%S+") do
            table.insert(tokens, token)
            if not token:match("^#%x%x%x%x%x%x$") then
                table.insert(wordIndices, #tokens)
                tokens[#tokens] = token:gsub("%d+", function(d) return d:reverse() end)
            end
        end

        for i = 1, math.floor(#wordIndices/2) do
            local j = #wordIndices - i + 1
            local idx1, idx2 = wordIndices[i], wordIndices[j]
...
```
local PersianSTR = fixPersianString("خب اینطوری به نظرم behtar az ghabl شد و دیگه کلماتabc1234 باهمدیگه قاطی 09120001209 نمیشن goodluck وقت بخیر ") 
local PersianSTR2 = fixPersianString([[درود #FF0000 بر #0000FFplayer2 #aaFFaa چخبرا؟
خوبم #FF0000    چطوری تو #ff8523 ؟player1 #aaFFaa ]])
outputChatBox(PersianSTR2,255,255,255,true)
local screenWidth, screenHeight = guiGetScreenSize ( )
addEventHandler ( "onClientRender", root, function ()
    dxDrawText ( PersianSTR, 200, screenHeight - 300, screenWidth, screenHeight, tocolor ( 255, 255, 255, 255 ), 2, "default-bold" )
    dxDrawText(PersianSTR2, 200, screenHeight - 500, screenWidth*2, screenHeight*2, tocolor(255, 255, 255, 255), 3, "default-bold", "left", "top", false, false, false, true)
end)
print(PersianSTR)
