---
doc_id: "mta-wiki:13558"
title: "AutoAttach"
source_title: "AutoAttach"
source_url: "https://wiki.multitheftauto.com/wiki/AutoAttach"
revision_id: 78395
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:25.204493+00:00"
---

# AutoAttach

This function attach an element into another by calculating the rotation and position offset automatically. It works exactly like [attachElements](mta://scripting/shared/functions/attachelements.md)  but you don't need to provide offset parameters.

## Syntax

```
bool autoAttach (element theElement, element theAttachToElement)
```
...
```
function autoAttach (theElement, theAttachToElement)
    if (theElement and theAttachToElement) then
        local mat = getElementMatrix (theAttachToElement)
        local vec = Vector3 (getElementPosition(theElement))
        local tx, ty, tz = getElementRotation (theAttachToElement)
        local vx, vy, vz = getElementRotation (theElement)
        ...
        local offY = vec.x * mat[1][2] + vec.y * mat[2][2] + vec.z * mat[3][2] + mat[4][2]
        local offZ = vec.x * mat[1][3] + vec.y * mat[2][3] + vec.z * mat[3][3] + mat[4][3]

        if (attachElements (theElement, theAttachToElement, offX, offY, offZ, rotX, rotY, rotZ)) then
            return true
        end
    end
    return false
...
This example attach a vehicle into another at the same position it is when the player uses the command /attach.  

It also detach the vehicle if the command is used again.  

This example creates a Packer (443) and a FCR-900 (521) at the center of the map. You need to be there to interact with them.

Click to collapse [-]
Server

```
theTruck = createVehicle (443, 0, 0, 4)
theVehicle = createVehicle (521, 0, 0, 7)

addCommandHandler ("attach", function (thePlayer, cmd)
    state = not state
    if (state) then
        autoAttach (theVehicle, theTruck)
        outputChatBox ("Bike attached to the truck", thePlayer, 0, 255, 0)
    else
        detachElements (theVehicle, theTruck)
        outputChatBox ("Bike detached from the truck", thePlayer, 255, 0, 0)
    end
end)
```
