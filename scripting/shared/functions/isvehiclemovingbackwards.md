---
doc_id: "mta-wiki:9858"
title: "IsVehicleReversing"
source_title: "IsVehicleMovingBackwards"
source_url: "https://wiki.multitheftauto.com/wiki/IsVehicleMovingBackwards"
revision_id: 76267
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:02.201569+00:00"
---

# IsVehicleReversing

This function checks if a vehicle is moving backwards or not. This is useful to create back lights on the vehicle.

## Syntax

```
bool isVehicleReversing(vehicle theVehicle)
```

### Required arguments

- **theVehicle:** The [vehicle](mta://reference/misc/vehicle.md) that you want to obtain the rear status of.

### Returns

This function returns *true* if the vehicle is going backwards, otherwise it returns *false* if the vehicle is going forwards or is stoped.
...
The client-side version is more trustable because it also checks the vehicle gear.
...
This example shows to the player if their current vehicle is going back or not.

```
function vehCheck ()
    local vehicle = getPedOccupiedVehicle (localPlayer)
    if vehicle then
        dxDrawText (tostring(isVehicleReversing(vehicle)), 10, 5) -- Show 'true' or 'false' on the screen.
    end
end
addEventHandler ("onClientRender", getRootElement(), vehCheck)
```

Example by: [LordHenry](https://wiki.multitheftauto.com/wiki/User:LordHenry)

Click to collapse [-]
Example 2

This example creates a white corona marker attached to the current vehicle when it is going back and set the marker invisible (size 0) when not.

```
rearLightL = {}
rearLightR = {}

function setVehicleRearLightState (theVehicle, light, state)
    local x, y, z = getElementPosition (theVehicle)
    if getElementModel (theVehicle) == 426 or getElementModel (theVehicle) == 420 then -- If is a Premier or a Taxi
        if light == 0 then
            if state == 0 then
                if not rearLightL[theVehicle] then
                    rearLightL[theVehicle] = createMarker (x, y, z, "corona", 0.1, 255, 255, 255, 255)
                end
                setMarkerSize (rearLightL[theVehicle], 0.1)
                attachElements (rearLightL[theVehicle], theVehicle, -0.85, -2.7, -0.05)
            elseif state == 1 then
        ...
        elseif light == 1 then
            if state == 0 then
                if not rearLightR[theVehicle] then
                    rearLightR[theVehicle] = createMarker (x, y, z, "corona", 0.1, 255, 255, 255, 255)
                end
                setMarkerSize (rearLightR[theVehicle], 0.1)
                attachElements (rearLightR[theVehicle], theVehicle, 0.85, -2.7, -0.05)
            elseif state == 1 then
    ...
    for i,v in ipairs (everyone) do
        ...
        if vehicle then
            if isVehicleReversing(vehicle) then
                setVehicleRearLightState (vehicle, 0, 0) -- Turns the 'back light' on.
                setVehicleRearLightState (vehicle, 1, 0)
            else
                setVehicleRearLightState (vehicle, 0, 1) -- Turns the 'back light' off.
                setVehicleRearLightState (vehicle, 1, 1)
            end
        end
    end
end
