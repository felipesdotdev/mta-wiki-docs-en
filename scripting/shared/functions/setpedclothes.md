---
doc_id: "mta-wiki:6681"
title: "SetPedClothes"
source_title: "SetPedClothes"
source_url: "https://wiki.multitheftauto.com/wiki/SetPedClothes"
revision_id: 76025
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:42.562661+00:00"
---
...
- **clothingSlot**: The clothing slot to change. See the [clothes catalog](mta://reference/misc/cj-clothes.md) for more info.

- **clothingID**: The clothing ID to set the slot to, or false to remove clothing from this slot (only applicable to slot IDs > 3). See the [clothes catalog](mta://reference/misc/cj-clothes.md) for more info.
...
```
local function clothesCommand(player, cmd, slot, id)
	if not tonumber(slot) then
	   outputChatBox("SYNTAX: /setClothes [slot] [id]", player, 255, 0, 0)
	   return
	end
	
	setPedClothes(player, tonumber(slot), id)
end
