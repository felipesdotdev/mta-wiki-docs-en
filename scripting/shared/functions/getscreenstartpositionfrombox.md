---
doc_id: "mta-wiki:12848"
title: "GetScreenStartPositionFromBox"
source_title: "GetScreenStartPositionFromBox"
source_url: "https://wiki.multitheftauto.com/wiki/GetScreenStartPositionFromBox"
revision_id: 81851
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:24.375610+00:00"
---

# GetScreenStartPositionFromBox

This function provides a way to calculate screen positions for UI elements in MTA:SA, particularly useful for positioning dx-effects and creating user interfaces. It uses a box-based positioning system that allows precise control over element placement relative to screen boundaries or center points.

The function calculates the starting position (X,Y coordinates) for drawing elements on the screen based on:

- A virtual box with defined dimensions (width/height)

- Desired positioning (left/right/center horizontally, top/bottom/center vertically)

- Offset values for fine-tuning the position
...
- **width:** The width of the virtual box in pixels

```
Can be 0, though using actual element dimensions is recommended
 Used for calculating center and right-aligned positions
```

- **height:** The height of the virtual box in pixels

```
Can be 0, though using actual element dimensions is recommended
 Used for calculating center and bottom-aligned positions
```

- **offsetX:** Horizontal offset from the calculated position

```
For "left"/"center": Positive values move right
 For "right": Positive values move left
```

- **offsetY:** Vertical offset from the calculated position

```
For "top"/"center": Positive values move down
 For "bottom": Positive values move up
```

### Optional Arguments

- **startIndicationX:** Horizontal positioning mode

```
Default: "left"
 Valid values:
   - "left": Aligns to left screen edge
   - "right": Aligns to right screen edge
   - "center": Centers horizontally on screen
```

- **startIndicationY:** Vertical positioning mode

```
Default: "top"
 Valid values:
   - "top": Aligns to top screen edge
   - "bottom": Aligns to bottom screen edge
   - "center": Centers vertically on screen
```
...
- **startX:** The calculated X coordinate for drawing

- **startY:** The calculated Y coordinate for drawing

### Behavior position calculation

- Left alignment: startX = offsetX

- Right alignment: startX = screenWidth - (width + offsetX)

- Center X alignment: startX = (screenWidth / 2) - (width / 2) + offsetX

- Top alignment: startY = offsetY

- Bottom alignment: startY = screenHeight - (height + offsetY)

- Center Y alignment: startY = (screenHeight / 2) - (height / 2) + offsetY
...
function ()
	...
	local offsetX = 100 * scaleValue
	local offsetY = 100 * scaleValue
	
	local startX, startY = getScreenStartPositionFromBox(rectangleWidth, rectangleHeight, offsetX, offsetY, "right", "bottom")
