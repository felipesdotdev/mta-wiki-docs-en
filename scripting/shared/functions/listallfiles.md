---
doc_id: "mta-wiki:14384"
title: "ListAllFiles"
source_title: "ListAllFiles"
source_url: "https://wiki.multitheftauto.com/wiki/ListAllFiles"
revision_id: 79962
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:04.596611+00:00"
---

# ListAllFiles

This function lists all files and subdirectories within a given directory and its subdirectories.

## Syntax

```
table listAllFiles ( string dirPath )
```

### Required Arguments

- **dirPath**: The string path of the directory to list files from.

### Returns

Returns a table containing the paths of all files in the given directory and its subdirectories.
...
function listAllFiles(dirPath)
    ...
    local function recursiveList(currentPath)
        local items = pathListDir(currentPath)
        for _, item in ipairs(items) do
            local itemPath = currentPath .. "/" .. item
            if pathIsFile(itemPath) then
                table.insert(fileList, itemPath)
            elseif pathIsDirectory(itemPath) then
                recursiveList(itemPath)
            end
        end
    end

    recursiveList(dirPath)
    return fileList
end
```
...
In this example, we will list all files in a specified directory and print them to the server console.

```
function listFilesCommandHandler(player, command, dirPath)
    if dirPath then
        local files = listAllFiles(dirPath)
        for _, file in ipairs(files) do
            outputChatBox(file, player)
        end
    else
        outputChatBox("Usage: /listfiles [directory_path]", player, 255, 0, 0)
    end
end
addCommandHandler("listfiles", listFilesCommandHandler)
```

By: [Xenius™](https://github.com/Xenius97)
