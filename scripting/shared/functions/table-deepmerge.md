---
doc_id: "mta-wiki:13289"
title: "Table.deepmerge"
source_title: "Table.deepmerge"
source_url: "https://wiki.multitheftauto.com/wiki/Table.deepmerge"
revision_id: 71660
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:55.320518+00:00"
---

# Table.deepmerge

This function deep merges two tables. Every nested table will be correspondingly merged.

## Syntax

```
table table.deepmerge( table from, table to[, table options] )
```

### Required Arguments

- **from**: The table which will be modified.

- **to**: The table with new values.

### Optional Arguments

- **options**: A table of options. Possible options are:

- **allowNew**: Allows previously nonexistent values to be created. If table **to** includes some key not defined in table **from** you can choose to include it in the result table. Default is true.

### Returns

Returns the **from** table merged with **to** table.
...
```
function table.deepmerge(from, to, options)
    ...
    for key, value in pairs(to) do
        ...
        if (from_type == "nil" and not options.allowNew) and -- It's okay to merge previous nonexistent values if 'allowNew' is specified
           from_type ~= to_type
        then
            error(string.format("'table.deepmerge' failed to merge incompatible types from '%s' to '%s' on key '%s'", from_type, to_type, key))
        end
...
Merges two tables allowing new values
...
Try to merge two tables but result in an error since key **b** doesn't exist in table **from**
...
**Hint**: This function can be useful for ensuring default values in a table
