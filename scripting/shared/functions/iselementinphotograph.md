---
doc_id: "mta-wiki:6845"
title: "IsElementInPhotograph"
source_title: "IsElementInPhotograph"
source_url: "https://wiki.multitheftauto.com/wiki/IsElementInPhotograph"
revision_id: 78414
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:54.819915+00:00"
---
...
```
function isElementInPhotograph(theElement)
    local nx, ny, nz = getPedWeaponMuzzlePosition(localPlayer)
    if (theElement ~= localPlayer) and (isElementOnScreen(theElement)) then -- Determine whether the element is even on the screen and not the client
        local px, py, pz = getElementPosition(theElement)
        local _, _, _, _, hitElement = processLineOfSight(nx, ny, nz, px, py, pz) -- Check if there is a collision between the "camera viewpoint" and the element
        if (hitElement == theElement) then -- If it collides with the element return true
            return true
        end
    end
    return false
...
local myPed = createPed(0, px + 1, py + 1, pz)
addEventHandler("onClientRender", root, function()
    dxDrawText(isElementInPhotograph(myPed) and "inPhotograph" or "notInPhotograph", 0, 0, 0, 0)
end)
```

**Adding Example by:** [Youssef Maged (iManGaaX)](https://wiki.multitheftauto.com/wiki/User:IManGaaX)
