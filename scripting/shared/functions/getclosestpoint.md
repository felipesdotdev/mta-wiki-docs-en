---
doc_id: "mta-wiki:14631"
title: "GetClosestPoint"
source_title: "GetClosestPoint"
source_url: "https://wiki.multitheftauto.com/wiki/GetClosestPoint"
revision_id: 82639
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:08.547407+00:00"
---

# GetClosestPoint

This function finds the closest point from a given element to a list of points in 2D space.

 

The difference between code No. 1 and code No. 2. The example in the image and below uses code No. 1.

| [[{{{image}}}\|link=\|]] | Note: The example in the image and below uses code No. 1. |
| --- | --- |
|  |  |
...
```
float, float, float getClosestPoint( element theElement, table thePoints )
```

### Required Arguments

- **theElement**: the element whose position is used as the reference.

- **thePoints**: a table containing coordinates in the form {x, y, z}.

### Returns

- Returns three *float*s indicating the position of the element, *x*, *y* and *z* respectively of the closest point.
...
```
function getClosestPoint(element, points)
    local ex, ey, ez = getElementPosition(element)
    local closestDist = math.huge
    local closestPoint = nil

    for _, p in ipairs(points) do
        local dx, dy = p[1] - ex, p[2] - ey
...
function getClosestPoint(element, points)
    ...
    for i = 1, #points - 1 do
        local x1, y1, z1 = unpack(points[i])
        local x2, y2, z2 = unpack(points[i + 1])

        local t = ((ex - x1) * (x2 - x1) + (ey - y1) * (y2 - y1)) / ((x2 - x1)^2 + (y2 - y1)^2)
...
```
local lanePoints = {
    {2469.95850, -1675.36279, 14},
    {2489.37866, -1653.80859, 14},
    {2508.62891, -1662.11462, 14},
}

local xyz = {getClosestPoint(localPlayer, lanePoints)} --// Code (1), See the images above
outputChatBox("Closest Lane Point: "..table.concat(xyz, ", "))
```
