---
doc_id: "mta-wiki:5538"
title: "Table.random"
source_title: "Table.random"
source_url: "https://wiki.multitheftauto.com/wiki/Table.random"
revision_id: 78088
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:16:56.902637+00:00"
---
...
addCommandHandler("random", function(player)
   outputChatBox("< "..table.random(randomGeT).." >", player)
end)
```
...
local iCanJoinEvent = {}
local eventMoney = 10000

outputChatBox("[Event] - Type /event to join the event and win "..(eventMoney).."$ [Event will ends in 1 minute]", root, 255, 255, 0)

setTimer(function()
    if (#eventParticipants > 0) then
        local randomPlayerFromTable = table.random(eventParticipants)
        givePlayerMoney(randomPlayerFromTable, eventMoney)
        outputChatBox("[Event] - Participants: "..(#eventParticipants)..", "..(getPlayerName(randomPlayerFromTable)).." win "..(eventMoney).."$", root, 255, 255, 0)
    else
        outputChatBox("[Event] - No one participated in the event", root, 255, 255, 0)
    end
    eventParticipants, iCanJoinEvent, eventMoney = nil
    removeCommandHandler("event")
end, 60000, 1)

addCommandHandler("event", function(thePlayer)
    if (not iCanJoinEvent[thePlayer]) then
        iCanJoinEvent[thePlayer] = true
        table.insert(eventParticipants, thePlayer)
        outputChatBox("[Event] - You have joined the event", thePlayer, 0, 255, 0)
    else
        outputChatBox("[Event] - You are already in the event", thePlayer, 255, 0, 0)
    end
end)
```
...
- Adding Example 2 by [Youssef Maged (iManGaaX)](https://wiki.multitheftauto.com/wiki/User:IManGaaX).
