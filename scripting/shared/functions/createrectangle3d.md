---
doc_id: "mta-wiki:13761"
title: "CreateRectangle3D"
source_title: "CreateRectangle3D"
source_url: "https://wiki.multitheftauto.com/wiki/CreateRectangle3D"
revision_id: 75172
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:38.400088+00:00"
---

# CreateRectangle3D

This function draws a 3d rectangle on the player screen.
...
```
bool createRectangle3D ( float x, float y, float width, float height, int color, float rx, float ry, float rz )
```
...
- **COLOR:** The color of the rectangle. color in RGBA format tocolor(red, green, blue, alpha)

- **RX:** The x rotation of the rectangle.

- **RY:** The y rotation of the rectangle.

- **RZ:** The z rotation of the rectangle.
...
local renderRectangle = dxCreateRenderTarget(resx, resy, true)
local theRectangleShader = dxCreateShader(rawShader)

function createRectangle3D(x, y, w, h, color, rx, ry, rz)
   local color = color or tocolor(255, 255, 255, 255)
   local rx, ry, rz = rx, ry, rz or 0, 0, 0
   if renderRectangle then
      dxSetRenderTarget(renderRectangle, true)
      dxDrawRectangle(x, y, w, h, color, false)
      dxSetRenderTarget()
      dxSetShaderValue(theRectangleShader, "texture", renderRectangle)
      dxSetShaderTransform(theRectangleShader, rx, ry, rz)
      dxDrawImage(x, y, w, h, theRectangleShader, 0, 0, 0, color, false)
   end
end
```
...
function TestDraw()
   ...
   createRectangle3D(200 * sx, 300 * sy, 300 * sx, 120 * sy, tocolor(255, 0, 0, 255), cx, cy, 0)
end
addEventHandler("onClientRender", root, TestDraw)
```
