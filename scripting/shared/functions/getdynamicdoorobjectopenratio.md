---
doc_id: "mta-wiki:13224"
title: "GetDynamicDoorObjectOpenRatio"
source_title: "GetDynamicDoorObjectOpenRatio"
source_url: "https://wiki.multitheftauto.com/wiki/GetDynamicDoorObjectOpenRatio"
revision_id: 77669
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:15:10.250103+00:00"
---

# GetDynamicDoorObjectOpenRatio

This function tells you how open a dynamic (interactive - pushable) door is (the 'open ratio') just like the [GetVehicleDoorOpenRatio](mta://scripting/shared/functions/getvehicledooropenratio.md) function.

| [[{{{image}}}\|link=\|]] | Note: This function only works properly on objects that have the special collision-response IDE flag #6 (SWINGDOOR). These are: 1491 , 1492 , 1494 , 1499 , 1502 , 1523 |
| --- | --- |
|  |  |
...
```
float getDynamicDoorObjectOpenRatio ( object theObject )
```

### Required Arguments

- **theObject:** The [object](mta://reference/misc/object.md) that you wish to get the door open ratio of.
...
```
function getDynamicDoorObjectOpenRatio(theObject)
    if isElement(theObject) then
        if getElementType(theObject) == "object" then
            local angularVelocity = select(3, getElementAngularVelocity(theObject))

            if math.abs(angularVelocity) > 0 then
                local objectMatrix = getElementMatrix(theObject)
                local initialAngle = select(3, getElementRotation(theObject, "ZXY"))
                local currentAngle = -math.deg(math.atan2(objectMatrix[2][1], objectMatrix[2][2]))

                if initialAngle + 180 >= currentAngle then
                    if initialAngle - 180 > currentAngle then
                ...
                return math.abs(currentAngle - initialAngle) / 108
            end
