---
doc_id: "mta-wiki:14187"
title: "DxDrawEditbox"
source_title: "DxDrawEditbox"
source_url: "https://wiki.multitheftauto.com/wiki/DxDrawEditbox"
revision_id: 78516
language: "en"
categories: ["Useful_Functions", "Utility_templates"]
generated_at: "2026-07-26T16:14:45.145232+00:00"
---

# DxDrawEditbox

This function draws a edit box across the screen - rendered for one frame. This should be used in conjunction with [onClientRender](mta://scripting/client/events/onclientrender.md) in order to display continuously.

## Syntax

```
bool dxDrawEditbox( string text, float startX, float startY, float width, float height, element editboxElement, [ int textColor = white, int maxCharacter = 10, bool isPassword = false, mixed font = "default", bool postGUI = false ] )
```

### Required Arguments

- **text**: if the edit box is empty this text will appear.

- **startX**: an float representing the **absolute** origin X position of the edit box, represented by pixels on the screen.

- **startY**: an float representing the **absolute** origin Y position of the edit box, represented by pixels on the screen.

- **width**: an float representing the width of the edit box, drawn in a *right* direction from the origin..

- **height**: an float representing the height of the edit box, drawn in a *downwards* direction from the origin.

- **editboxElement**: the element of edit box, should be create a element under name **dxDrawEditbox** before use function **dxDrawEditbox** (look at the example).

### Optional Arguments

*NOTE:* When using optional arguments, you might need to supply all arguments before the one you wish to use. For more information on optional arguments, see [optional arguments](mta://reference/misc/optional-arguments--f0d8694a.md).

- **textColor**: the hex color of the text on edit box, produced using [tocolor](mta://scripting/shared/functions/tocolor.md) or 0xAARRGGBB (AA = alpha, RR = red, GG = green, BB = blue).

- **maxCharacter**: the maximum number of characters in the edit box.

- **isPassword**: is the text on the edit box a password? change it to true to change the speech into symbols.
...
- **postGUI**: a bool representing whether the edit box should be drawn on top of or behind any ingame GUI (rendered by CEGUI).
...
function dxDrawEditBox(text, startX, startY, width, height, editboxElement, textColor, maxCharacter, isPassword, font, postGUI)
    local editboxText = getElementData(editboxElement, "editboxText") or setElementData(editboxElement, "editboxText", "")
    local editboxState = getElementData(editboxElement, "editboxState") or setElementData(editboxElement, "editboxState", false)
    local editboxCharacter = getElementData(editboxElement, "editboxMaximum") or setElementData(editboxElement, "editboxMaximum", maxCharacter or 10)

    if type(editboxText) == "boolean" then return false end

    dxDrawText(#editboxText == 0 and text or (isPassword and string.gsub(editboxText, ".", "•") or editboxText), startX, startY, width + startX, height + startY, textColor or tocolor(255, 255, 255, 255/1.2), 1, font or "default", "center", "center", true, false, postGUI or false)
    dxDrawLine(startX, startY + height, width + startX, height + startY, tocolor(255/3, 255/3, 255/3, editboxState and math.abs(math.sin(getTickCount() / 255) * 255) or 255), 1, postGUI or false)

    if isCursorShowing() then
        local screen, cursor = {guiGetScreenSize()}, {getCursorPosition()}
        local cx, cy = cursor[1] * screen[1], cursor[2] * screen[2]
        if (cx >= startX and cx <= startX + width) and (cy >= startY and cy <= startY + height) then
    ...
    if (isValidPosition == nil) then return false end

    if (isValidPosition) then
        setElementData(editboxElement, "isMouseState", "in")
    else
        setElementData(editboxElement, "isMouseState", "out")
    end
...
local function dxDrawEditBoxClickElement(button, state)
    if (button == "left" and state == "down") then
        ...
        local function toggleStatus(element, theState)
            ...
            setElementData(element, "editboxState", theState)
        end

        for index, element in ipairs(getElementsByType("dxDrawEditbox")) do
            if (getElementData(element, "isMouseState") == "in") then
                editboxID = index
            elseif (getElementData(element, "isMouseState") == "out") and (getElementData(element, "editboxState") == true) then
                toggleStatus(element, false)
            end
        end

        if (not editboxID) then return end

        local editbox = getElementsByType("dxDrawEditbox")[editboxID]
        if isElement(editbox) and (getElementData(editbox, "editboxState") == false) then
            toggleStatus(editbox, true)
        end
    end
end
...
local function dxDrawEditBoxCharacterElement(button)
    if (editboxStatus) and (not isChatBoxInputActive()) and (not isConsoleActive()) then
        for index, element in ipairs(getElementsByType("dxDrawEditbox")) do
            if (getElementData(element, "editboxState") == true) and (#getElementData(element, "editboxText") < getElementData(element, "editboxMaximum")) then
                setElementData(element, "editboxText", getElementData(element, "editboxText")..button)
            end
        end
    end
end
...
local function dxDrawEditBoxKeyElement(button, press)
    if (editboxStatus) and (not isChatBoxInputActive()) and (not isConsoleActive()) and (press) and (button == "backspace") then
        for index, element in ipairs(getElementsByType("dxDrawEditbox")) do
            if (getElementData(element, "editboxState") == true) and (#getElementData(element, "editboxText") > 0) then
                setElementData(element, "editboxText", utf8.remove(getElementData(element, "editboxText"), -1, -1))
            end
        end
    end
end
...
**Author:** [Youssef Maged (iManGaaX)](https://wiki.multitheftauto.com/wiki/User:IManGaaX)
...
```
local isScreen = {guiGetScreenSize()}
local devX, devY = (isScreen[1] / 1600), (isScreen[2] / 900)
...
addEventHandler("onClientRender", root, function()
    dxDrawEditBox("Username", devX * 700, devY * 400, devX * 200, devY * 35, ebElements[1], tocolor(255, 0, 0, 255), 10, false, "default", true)
    dxDrawEditBox("Password", devX * 700, devY * 450, devX * 200, devY * 35, ebElements[2], tocolor(0, 255, 0, 255), 20, true, "default", true)
end)

addCommandHandler("editboxvalid", function()
    if #getElementData(ebElements[1], "editboxText") == 0 then
        outputChatBox("the username edit box is empty")
    elseif #getElementData(ebElements[2], "editboxText") == 0 then
        outputChatBox("the password edit box is empty")
    elseif #getElementData(ebElements[2], "editboxText") < 4 then
        outputChatBox("the password should be >= 4 characters")
    else
        --// Send trigger now [EX].
    end
end)
