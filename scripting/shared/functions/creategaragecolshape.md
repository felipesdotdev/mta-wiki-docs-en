---
doc_id: "mta-wiki:13894"
title: "CreateGarageColShape"
source_title: "CreateGarageColShape"
source_url: "https://wiki.multitheftauto.com/wiki/CreateGarageColShape"
revision_id: 76275
language: "en"
categories: ["Useful_Functions"]
generated_at: "2026-07-26T16:10:37.736530+00:00"
---

# CreateGarageColShape

This function creates a collision shape from the specified garage. Most garage collision shapes are rectangles, but if the garage is diagonal, its collision shape is a polygon.
...
```
colshape createGarageColShape (int garageID)
```

### Required Arguments

- **garageID:** The [garage ID](mta://reference/misc/garage.md) that represents the garage interior that's being gotten.
...
```
function createGarageColShape (id)
    local sizeZ, sizeX, sizeY = getGarageSize (id) -- The Z size starts from Z position = 0 to the garage ceil. That's why you need to recalculate later.
    local posX, posY, posZ = getGaragePosition (id) -- This is the south-west bottom corner of the garage.
    if not sizeZ or not posX then return false end
    sizeZ = sizeZ - posZ -- Fix sizeZ.
    if id == 14 or id == 44 or id == 46 then -- If the garage is diagonal, then:
        local dummy = createObject (3003, posX, posY, posZ)
        ...
        if id == 14 then
            sizeZ = sizeZ - 1.5 -- Fix GTA native issue.
            setElementRotation(dummy, 0, 0, 19)
        elseif id == 44 then
            setElementRotation(dummy, 0, 0, 15)
        elseif id == 46 then
            sizeX = sizeX + 1 -- Fix GTA native issue.
            setElementRotation(dummy, 0, 0, 321)
        end
        
        local x1, y1 = getPositionFromElementOffset(dummy, sizeX, 0, 0)
        local x2, y2 = getPositionFromElementOffset(dummy, sizeX, sizeY, 0)
        local x3, y3 = getPositionFromElementOffset(dummy, 0, sizeY, 0)
        
        local shape = createColPolygon (posX, posY, posX, posY, x1, y1, x2, y2, x3, y3)
        setColPolygonHeight(shape, posZ, posZ+sizeZ)
        destroyElement(dummy)
        return shape
    else -- If the garage is not diagonal, then:
        return createColCuboid (posX, posY, posZ, sizeX, sizeY, sizeZ)
    end
end
```
...
```
setGarageOpen (9, true) -- Open CJ house garage door
createGarageColShape (9) -- Create the garage colShape.
```
