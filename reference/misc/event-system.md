---
doc_id: "mta-wiki:1851"
title: "Event system"
source_title: "Event system"
source_url: "https://wiki.multitheftauto.com/wiki/Event_system"
revision_id: 62258
language: "en"
categories: ["Scripting_Concepts"]
generated_at: "2026-07-26T16:14:59.766367+00:00"
---
...
To use the event system, you attach event handlers to elements in the element tree using [addEventHandler](mta://scripting/shared/functions/addeventhandler.md). When you do this, your function will get triggered for all the events triggered on that element, it's parents (and their parents, etc.) and it's children (and their children). As such, an event handler attached to the *root* element will be triggered when an event occurs for any element. As a consequence you should generally use as specific a handler as you can. If you wish to just see when the player enters a specific marker, just attach the event handler to that marker.
...
The function you attached to an event gets called and passed a bunch of arguments. These arguments are event-specific. Each event has specific parameters, for instance [onClientGUIClick](mta://scripting/client/events/onclientguiclick.md) has 4 parameters, which are:
...
MTA has a number of built in events. These are listed on the pages [Client Scripting Events](mta://scripting/concepts/client-scripting-events.md) and [Server Scripting Events](mta://scripting/concepts/server-scripting-events.md).

## Custom events

You can create your own events that can be triggered across all resources. This is an important way to communicate with other resources and allow them to hook into your code. To add your own custom event, just call the [addEvent](mta://scripting/shared/functions/addevent.md) function. You can then use the [triggerEvent](mta://scripting/shared/functions/triggerevent.md) function to trigger that event any time you want - either using a timer, or based on a more general event.

For example, you could be making a Capture the Flag game mode and want to trigger an event when a player captures the flag. You could do this by attaching a event handler to the standard MTA [onMarkerHit](mta://scripting/server/functions/onmarkerhit.md) event and checking that the player entering the marker has the flag. if they do, you can then trigger your more specific *onFlagCaptured* event and other resources could handle this as they please.

## Canceling

Events can be canceled with [cancelEvent](mta://scripting/shared/functions/cancelevent.md). This can have a variety of effects, but in general this means that the server will not perform whatever action it would usually do. For example, canceling [onPickupUse](mta://scripting/server/events/onpickupuse.md) would prevent a player being given what they tried to pick up, canceling [onVehicleStartEnter](mta://scripting/server/events/onvehiclestartenter.md) would prevent the player entering the vehicle. You can check if the currently active event has been canceled using [wasEventCanceled](mta://scripting/shared/functions/waseventcanceled.md). It's important to note that canceling event *does not* prevent other event handlers being triggered.
