...
- Methods can either start like *[player](https://wiki.multitheftauto.com/index.php?search=player):* or *[Player](https://wiki.multitheftauto.com/index.php?search=Player).* - the former is only for a function on an instance (setPlayerHealth) and the latter is a static method (getRandomPlayer).
