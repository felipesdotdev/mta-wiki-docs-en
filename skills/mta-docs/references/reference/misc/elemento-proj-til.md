---
doc_id: "mta-wiki:12072"
title: "Elemento/Projétil"
source_title: "Elemento/Projétil"
source_url: "https://wiki.multitheftauto.com/wiki/Elemento/Proj%C3%A9til"
revision_id: 65379
language: "en"
categories: ["Element_Types"]
---

# Elemento/Projétil

A classe projétil representa os projetéis de certas armas que são criados quando elas os lançam. Mas também esses projéteis podem ser criados usando a função  [createProjectile](mta://scripting/client/functions/createprojectile.md).

- **16: Granada**

- **17: Granada de gás lacrimogêneo**

- **18: Molotov**

- **19: Foguete (comum)**

- **20: Foguete (sensor de calor)**

- **21: Air Bomb**

- **39: Carga de Satchel**

- **58: Fogo do Hydra (aquela luz brilhante que o Hydra libera)**

## Veja também

- [createProjectile](mta://scripting/client/functions/createprojectile.md)

- [getProjectileCounter](mta://scripting/client/functions/getprojectilecounter.md)

- [getProjectileCreator](mta://scripting/client/functions/getprojectilecreator.md)

- [getProjectileForce](mta://scripting/client/functions/getprojectileforce.md)

- [getProjectileTarget](mta://scripting/client/functions/getprojectiletarget.md)

- [getProjectileType](mta://scripting/client/functions/getprojectiletype.md)

- [setProjectileCounter](mta://scripting/client/functions/setprojectilecounter.md)
  

- **Shared**

- [detonateSatchels](mta://scripting/shared/functions/detonatesatchels.md)
