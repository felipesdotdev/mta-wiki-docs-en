---
doc_id: "mta-wiki:1871"
title: "Known Issues - FAQ"
source_title: "Known Issues - FAQ"
source_url: "https://wiki.multitheftauto.com/wiki/Known_Issues_-_FAQ"
revision_id: 78757
language: "en"
categories: ["Support"]
---
...
| [[{{{image}}}\|link=\|]] | Important Note: Before you check the known issues make sure everything is Up-to-date . |
| --- | --- |
|  |  |
...
- [Map editor FAQ/known issues can be found here.](mta://resources/editor.md)

- [Known issues for MTA 0.5r2 can be found here.](mta://reference/misc/mta-0-5r2-known-issues.md)

- [Compatibility FAQ](mta://getting-started/compatibility-faq.md)

## Client

### General

#### I have the Steam version of GTA San Andreas. How can I play MTA?

The Steam version of GTA:SA is compatible with MTA, as it already has a built-in patcher.
...
If you want to play some older MTA version (1.3.2 or older), follow a few simple steps, and the Steam version will be compatible with it:
...
This procedure will not affect your Steam version of GTASA, but will allow MTA to boot alongside it.

- **Option 2:** Use the unofficial [steam downgrade patch](https://forums.steampowered.com/forums/showthread.php?t=1952458), be sure to downgrade to 1.0 and not 1.01.

#### Does MTA work with v1.01 or v2.00 of GTA San Andreas?

Yes, they are compatible with MTA, as it already has a built-in patcher.
...
If it all fails and you run Kaspersky Anti-Virus or Internet Security, make sure status of "multi theft auto.exe" is not restricted. Other anti-virus software may block MTA from running.

#### Crash after MTA splash screen

- **MTA crashes after the 'Stop playing with yourself' logo. Both single-player and the MTA ran fine before.**

Try downloading the latest DirectX Runtime files from [Microsoft](https://www.microsoft.com/downloads/details.aspx?FamilyID=2da43d38-db71-4c1b-bc6a-9b6652cd92a3&DisplayLang=en). Also check in Task Manager, if :gta_sa.exe process isn't already running.
...
- Enable Windows XP SP3 compatibility mode for both Multi Theft Auto.exe and gta_sa.exe, setting their privilege level to "Run this program as an administrator".
...
- Run MTA as administrator.

#### Crash after connecting to any server

- **MTA crashes upon connecting to any server. Single player runs fine.**

Single-player mods can affect the way MTA works, potentially causing crashes - you should always use a clean GTASA install for MTA.
...
Ensure that your controller is the first controller recognized by Windows (MTA will only use the first controller).  You can configure your gamepad in options in MTA's main menu.
...
If that doesn't work try starting GTA in Singleplayer, go to options > controller setup and set "Configuration" to "Mouse + Keys" instead of "Joypad".
...
You are running an outdated version of Multi Theft Auto. Head over to the [main page](https://mtasa.com/) and download the latest version of Multi Theft Auto.

#### 'Network module not compatible!' on MTA launch

- **I am getting 'Network module not compatible!' error message upon launching MTA.**

This could mean that your MTA install is incomplete or broken. Reinstall it.
...
To install/update DirectX download the [DirectX End-User Runtime Web Installer](https://www.microsoft.com/downloads/details.aspx?displaylang=en&FamilyID=2da43d38-db71-4c1b-bc6a-9b6652cd92a3) from the Microsoft download site.
...
#### 'Error loading netc.dll module! (Error 14001)' on MTA launch
...
[Microsoft Visual C++ 2008 Redistributable Package (x86)](https://www.microsoft.com/downloads/details.aspx?FamilyID=9b2da534-3e03-4391-8a4d-074b9f2bc1bf&displaylang=en)

[Microsoft Visual C++ 2008 SP1 Redistributable package (x86)](https://www.microsoft.com/download/en/details.aspx?id=5582)

#### 'Error 1935. An error occurred during the installation of assembly 'Microsoft.VC90.ATL...'

Download the 'Fix it' helper from here: [[1]](https://support.microsoft.com/default.aspx?scid=kb;en-us;946414)

#### Your virus scanner warns you about MTA

- **Your virus scanner informs you that the MTA or setup executable is a virus or malware.**

MTA does **NOT** contain any viruses, malware, adware or spyware. You should re-download MTA [here](https://mtasa.com) if you doubt the validity of your copy of MTA.
...
This issue is fixed in [MTA 1.3](https://forum.mtasa.com/topic/33297-multi-theft-auto-san-andreas-13-released/).

#### loader.dll (error 126)

loader.dll (error 126) has been flagged as a false positive by Avast, McAfee and perhaps other antivirus software. This means that your antivirus software thinks it's a virus, but it actually isn't.
...
- Browse to, and select, the file that you want to exclude from scans.
...
#### I can only set my display to square resolutions...

Try reinstalling your display driver.

### Windows Vista® / Windows 7-specific (Vista is no longer supported on 1.5.7)

#### Crash on connect

- **I seem to crash whenever I connect to a server just before I go in-game on Vista**

This seems to be an issue with the Microsoft DirectX April 2006 SDK Redistributable DLL file (d3dx9_30.dll) when running in compatibility mode. Please make sure that compatibility mode is completely turned off for  **both** your GTA_SA.exe and Multi Theft Auto.exe executables.

#### Clock manipulation error

- **I am getting 'Clock manipulation detected!' error message upon launching MTA:SA**

This is caused by incorrect system date/time is set (which could be a result of wrong settings or a faulty battery on the pc's motherboard). Setting time and date again should fix the problem.

It might also happen if you are using an AMD Athlon 64 X2 processor with some old drivers. Update them at [AMD's site](https://support.amd.com/en-us/download).

#### Halt on launch

- **When I launch MTA:SA, nothing happens (GTA_SA.exe is running but not loading up)**

Run MTA:SA with Administrator privileges. To do this, right-click on the installer executable, choose 'Properties', go into 'Compatibility' tab and tick the checkbox on the last field and try again.

#### General GTA problems

- **I have unexplainable GTA problems or crashes**

Make sure your computer, as well as your GTA install, meet the [minimum requirements](https://wiki.multitheftauto.com/index.php?search=minimum%20requirements) and that you are not running in any 98/2000/XP/2003 compatibility modes.

Also try the solutions from these pages:

- [http://gtaforums.com/topic/307992-san-andreas-v1-works-on-windows-vista/](http://gtaforums.com/topic/307992-san-andreas-v1-works-on-windows-vista/)

- [http://pullmonkey.com/2007/4/30/how-i-got-gta-san-andreas-to-work-with-a-crappy-os-vista](https://web.archive.org/web/20160330172922/http://pullmonkey.com/2007/4/30/how-i-got-gta-san-andreas-to-work-with-a-crappy-os-vista)
...
There seems to be an issue in combination with the Windows power profile running in power saving mode, you can solve this issue by changing the power profile to high performance when playing MTA:SA.

- Topic about this issue: [Sandy Bridge performance issues?](https://forum.mtasa.com/topic/26441-solved-sandy-bridge-performance-issues/)
...
- If you have configured an external web server (**httpdownloadurl** is set to your custom URL), make sure that your HTTP is accessible and make sure you have read the [Configuring an external web server](https://wiki.multitheftauto.com/index.php?search=Configuring%20an%20external%20web%20server) guide.
...
Account information (or anything else) not being saved correctly can be caused by database corruption. From build 2836, the server checks the integrity of the SQLite database files on startup. If it reports a database error, please read [How to repair the database files](mta://reference/misc/how-to-repair-the-database-files.md).

If you are running an earlier version and are having troubles such as accounts disappearing, you should upgrade your server to the [latest build](https://nightly.multitheftauto.com/)
