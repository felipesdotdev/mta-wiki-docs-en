---
doc_id: "mta-wiki:5027"
title: "Introduction to Scripting the GUI - Part 2"
source_title: "Introduction to Scripting the GUI - Part 2"
source_url: "https://wiki.multitheftauto.com/wiki/Introduction_to_Scripting_the_GUI_-_Part_2"
revision_id: 60545
language: "en"
categories: ["GUI_Tutorials"]
---

# Introduction to Scripting the GUI - Part 2

In this tutorial we will make a simple rules window, with an accept button and a list of all the server rules & information.

**Note that this tutorial builds on content covered in the [GUI Scripting Introduction](mta://scripting/concepts/introduction-to-scripting-the-gui.md).**
...
For this tutorial we will be using one [window](mta://scripting/concepts/element-gui-window.md), one [button](mta://scripting/concepts/element-gui-button.md) and one [label](mta://scripting/concepts/element-gui-text-label.md).
We will be using **absolute** position values.

As noted in the [Previous tutorial](mta://scripting/concepts/introduction-to-scripting-the-gui.md), all the GUI must be made client side.
...
In this file we will write a function that draws the window:
...
Therefore, we'll write an event handler for [onClientResourceStart](mta://scripting/client/events/onclientresourcestart.md) to create the window:
...
This can be done using the same event, [onClientResourceStart](mta://scripting/client/events/onclientresourcestart.md), so we can modify the above code to include showing the cursor:
...
In our [onClientGUIClick](mta://scripting/client/events/onclientguiclick.md) event handle, we told it to call the function 'acceptRules' whenever 'rulesButton' is clicked.
...
However, if they do not accept it they can stay in the server indefinitely with the window open.

### Setting a timer

To combat this, we will add a timer using [setTimer](mta://scripting/shared/functions/settimer.md) to kick them from the server after a period of time if they have not accepted the rules.
...
Put this [setTimer](mta://scripting/shared/functions/settimer.md) line at the bottom of your 'createRulesWindow' function.
...
Note the use of [triggerServerEvent](mta://scripting/client/functions/triggerserverevent.md) to call the server. You should have experience with client-server interaction from the [Previous tutorial](mta://scripting/concepts/introduction-to-scripting-the-gui.md). 
...
Note the use of [addEvent](mta://scripting/shared/functions/addevent.md) and [addEventHandler](mta://scripting/shared/functions/addeventhandler.md) on the server. You should have experience with client-server interaction from the [Previous tutorial](mta://scripting/concepts/introduction-to-scripting-the-gui.md). 
If not, you can go back and read the full explanation there.
...
**Note that [kickPlayer](mta://scripting/server/functions/kickplayer.md) will require your resource to have kick access in your ACL.**
...
For more information on the [ACL](https://wiki.multitheftauto.com/index.php?search=ACL), see the [ACL](https://wiki.multitheftauto.com/index.php?search=ACL) wiki page.
