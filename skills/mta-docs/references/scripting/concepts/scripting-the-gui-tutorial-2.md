---
doc_id: "mta-wiki:5024"
title: "Scripting the GUI - Tutorial 2"
source_title: "Scripting the GUI - Tutorial 2"
source_url: "https://wiki.multitheftauto.com/wiki/Scripting_the_GUI_-_Tutorial_2"
revision_id: 60929
language: "en"
categories: ["GUI_Tutorials"]
---

# Scripting the GUI - Tutorial 2
...
**Note that this tutorial assumes you are familiar with all the content covered in the [GUI Scripting Introduction](https://wiki.multitheftauto.com/index.php?search=GUI%20Scripting%20Introduction).**
...
To begin, open up a clientside lua file (if you have been following the [GUI Scripting Introduction](https://wiki.multitheftauto.com/index.php?search=GUI%20Scripting%20Introduction), this is gui.lua) to work with.

### Creating the GUI

By now, GUI creation should seem relatively straight forward, so we will not go over this in too much detail.
As in [Tutorial 1](mta://scripting/concepts/scripting-the-gui-tutorial-1.md) we will be using **absolute** values in this tutorial.
...
We could add [onClientGUIClick](mta://scripting/client/events/onclientguiclick.md) events for every button individually, as we have done in previous tutorials; Or, alternatively, 
we could add a single [onClientGUIClick](mta://scripting/client/events/onclientguiclick.md) handler for the window (which is the parent of all our other keypad GUI elements), filter our results to include only the buttons we want and trigger our own custom event.
...
Much like when using [triggerServerEvent](mta://scripting/client/functions/triggerserverevent.md) in previous tutorials, we now need to define the event, only this time it is purely clientside:
...
Note the second argument in [addEvent](mta://scripting/shared/functions/addevent.md) is set to false, indicating that this event cannot be triggered from the server.
Also note, the function we are using in the [addEventHandler](mta://scripting/shared/functions/addeventhandler.md) does not have a name. If you contract it down and remove the spacing, you get:
...
*If you are unsure of map file syntax, please see the [Writing Gamemodes Tutorial](mta://mapping/writing-gamemodes.md) or check the [Object](https://wiki.multitheftauto.com/index.php?search=Object) page for help.*
...
For further GUI tutorials, see [Tutorial 3 (Scrolling News Feed)](mta://scripting/concepts/scripting-the-gui-tutorial-3.md)
