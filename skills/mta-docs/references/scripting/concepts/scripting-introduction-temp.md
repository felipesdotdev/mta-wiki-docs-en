...
Our one-line script runs immediately when the resource starts, but games are interactive. You usually want code to trigger only when a specific action happens in the world—like a player entering a vehicle or taking damage. We handle this using [Events](https://wiki.multitheftauto.com/index.php?search=Events).
...
Notice a big difference? In client-side commands, we don't need `thePlayer` as the first function argument. This is because on the client side, the script is only running for one person, so the player element is always automatically the [localPlayer](https://wiki.multitheftauto.com/index.php?search=localPlayer).
