---
doc_id: "mta-wiki:10503"
title: "CustomWidget:getRealSize"
source_title: "CustomWidget:getRealSize"
source_url: "https://wiki.multitheftauto.com/wiki/CustomWidget%3AgetRealSize"
revision_id: 57140
language: "en"
...
This function returns Custom Widget real size on screen.

## Syntax

```
float, float CustomWidget:getRealSize([bool Relative])
```

### Other Arguments

- **Relative** - Relative of size (if false, size will be absolute). Default - *false*.

### Returns

Returns Real Widget's Width and Height.

## Information

Some widgets have difference between original widget size and it's real onscreen size.

For example - window widget when creating sets it's frame size to size, sended in arguments, and creates shadows of window widget with 2 pixels on side. Summary - real size is greater than original for 4 pixels in Width and Height.

## Example

```
local Window = CustomWindow.create(0, 0, 100, 100, "Example", false)
local w, h = Window:getSize()
local aw, ah = Window:getRealSize()

outputChatBox((aw-w).." : "..(ah-h)) --Prints "4 : 4"
```
