---
doc_id: "mta-wiki:6791"
title: "GetVehicleComponentPosition"
source_title: "GetVehicleComponentPosition"
source_url: "https://wiki.multitheftauto.com/wiki/GetVehicleComponentPosition"
revision_id: 82790
language: "en"
...
This function gets the component position of a [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle). The vehicle must be streamed in.
...
**Method**: *[vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle):getComponentPosition(...)*

**Counterpart**: *[setVehicleComponentPosition](mta://scripting/client/functions/setvehiclecomponentposition.md)*

### Required Arguments

- **theVehicle:** The [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle) you wish to get component position of.
...
Rectify the warning code and render it.
...
addEventHandler("onClientRender", root, function()
    ...
    if not playerVeh then return end -- If we're not in the car, we don't do anything
    -- We only check the components of your machine
    local components = getVehicleComponents(playerVeh)
    if components then
        for compname in pairs(components) do
            local px, py, pz = getVehicleComponentPosition(playerVeh, compname, "world")
            if px then
                local sx, sy = getScreenFromWorldPosition(px, py, pz)
                if sx then
                    local text = string.format("%s\n[%.2f, %.2f, %.2f]", compname, px, py, pz)
                    dxDrawText(text, sx, sy, sx, sy, tocolor(255, 255, 0, 255), 0.8, "default-bold", "center", "bottom")
                end
            end
        end 
    end
