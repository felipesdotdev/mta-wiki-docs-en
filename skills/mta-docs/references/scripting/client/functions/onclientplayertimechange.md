---
doc_id: "mta-wiki:14162"
title: "OnClientPlayerTimeChange"
source_title: "OnClientPlayerTimeChange"
source_url: "https://wiki.multitheftauto.com/wiki/OnClientPlayerTimeChange"
revision_id: 82585
language: "en"
categories: ["Useful_Functions"]
---

# OnClientPlayerTimeChange

This code implements an [event](https://wiki.multitheftauto.com/index.php?search=event) that is triggered when the [player](https://wiki.multitheftauto.com/index.php?search=player)'s real time change..

| [[{{{image}}}\|link=\|]] | Note: This event checks the player's time as set on the computer and not the region time or GTA's time. |
| --- | --- |
|  |  |
...
The [source](mta://reference/misc/event-system.md) of this event is the player whose real time changed.
...
```
setTimer(function()
    local myTime = getRealTime()
    triggerEvent("onClientPlayerTimeChange", localPlayer, myTime.hour, myTime.minute)
    setTimer(function()
        local myTime = getRealTime()
        triggerEvent("onClientPlayerTimeChange", localPlayer, myTime.hour, myTime.minute)
    end, 60000, 0)
end, (60-(getRealTime().second))*1000, 1)
addEvent("onClientPlayerTimeChange", false)
```
...
Example 1: This example will set GTA's clock like as player's real time, **onClientPlayerTimeChange** is function is to change the GTA's time when the real time is changed.

Example 2: This example will make label with player's real time, **onClientPlayerTimeChange** is function is to change the time on label when the real time is changed.
...
addEventHandler("onClientPlayerTimeChange", localPlayer, function(hour, minute)
    setTime(hour, minute)
end)
...
```
local myCurrentTimeLabel

addEventHandler("onClientResourceStart", resourceRoot, function()
    local realTime = getRealTime()
    local text = string.format("%02d:%02d", realTime.hour, realTime.minute)
    myCurrentTimeLabel = guiCreateLabel(0, 0, 0.02, 0.02, text, true)
end)

addEventHandler("onClientPlayerTimeChange", localPlayer, function(hour, minute)
    local text = string.format("%02d:%02d", hour, minute)
    guiSetText(myCurrentTimeLabel, text)
end)
```
