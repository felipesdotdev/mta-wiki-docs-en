---
doc_id: "mta-wiki:10770"
title: "CustomWidget:destroy"
source_title: "CustomWidget:destroy"
source_url: "https://wiki.multitheftauto.com/wiki/CustomWidget%3Adestroy"
revision_id: 58854
language: "en"
...
This function destroying Custom Widget.

## Syntax

```
nil CustomWidget:destroy()
```

## Information

After destroying object, variable still has methods, but GUI is destroying. Suppose that developer will never use variable of destroyed object, and set it to **nil** after destroying. Any methods working on destroyed object, but it will sends error in debug, because objects is doesn't exists.

## Example

This example creating window with button, and then destroys window:

```
local Window = CustomWindow.create(100, 100, 200, 200, "Example", false)
local Button = CustomButton.create(50, 25, 100, 30, "Button", false, Window)

local Event = Window:addEvent("onClientGUIClick", function()
	outputChatBox("You've clicked on window")
end)

Window:removeEvent("onClientGUIClick", Event)

Window:destroy()
Window = nil --Suppose that developer will never use this variable as object, what destroyed in previous step
```
