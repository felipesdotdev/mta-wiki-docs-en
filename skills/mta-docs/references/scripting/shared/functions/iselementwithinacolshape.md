---
doc_id: "mta-wiki:7200"
title: "IsElementWithinAColShape"
source_title: "IsElementWithinAColShape"
source_url: "https://wiki.multitheftauto.com/wiki/IsElementWithinAColShape"
revision_id: 49469
language: "en"
...
This function checks if the [element](mta://reference/misc/element.md) is in a [colshape](https://wiki.multitheftauto.com/index.php?search=colshape).
...
Returns a [colshape](https://wiki.multitheftauto.com/index.php?search=colshape) the [element](mta://reference/misc/element.md) is in else false if the element isn't in one or no arguments were passed.
...
This code will destroy the colshape the player is currently in when he types the command '/deletecol'.

```
function deleteColShape(thePlayer, command)
   local colshape = isElementWithinAColShape(thePlayer)
   if colshape then -- if the player who entered the command is in a colshape
      outputChatBox("The colshape you are in has been successfully deleted!", thePlayer, 0, 255, 0)
      destroyElement(colshape) -- destroy the col shape he is in
   else
      outputChatBox("You are currently not in a colshape", thePlayer, 255, 0, 0) -- if he is not in the colshape
   end
end
addCommandHandler("deletecol", deleteColShape)
```
