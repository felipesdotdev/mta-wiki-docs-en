---
doc_id: "mta-wiki:14657"
title: "SetVehicleDirtEnabled"
source_title: "SetVehicleDirtEnabled"
source_url: "https://wiki.multitheftauto.com/wiki/SetVehicleDirtEnabled"
revision_id: 82688
language: "en"
categories: ["Useful_Functions"]
---

# SetVehicleDirtEnabled

This function toggles a dirt removal [shader](https://wiki.multitheftauto.com/index.php?search=shader) effect on the specified [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle).

 

| [[{{{image}}}\|link=\|]] | Note: This function applies or removes a custom shader that removes dirt texture from the vehicle 's surface. It also handles shader cleanup automatically when the vehicle is destroyed. |
| --- | --- |
|  |  |
...
```
bool setVehicleDirtEnabled( element theVehicle, boolean enabledState )
```

### Required Arguments

- **theVehicle**: the vehicle element on which to toggle the dirt effect.

- **enabledState**: set to false to remove dirt, true to show default dirt.

### Returns

- **true** if the operation was successful (shader applied or removed, or already in the requested state).

- **false** if the operation failed (shader creation failure).
...
local shaderCode = [[texture removeDirtyTexture;
technique TexReplace {
    pass P0 {
        Texture[0] = removeDirtyTexture;
    }
}]]

function isVehicleDirtEnabled(theVehicle)
    assert(isElement(theVehicle) and getElementType(theVehicle) == "vehicle", "Bad argument @ 'isVehicleDirtEnabled' [Expected vehicle element at argument 1, got "..(isElement(theVehicle) and getElementType(theVehicle) or type(theVehicle)).."]")

    if isElement(enabledVehicles[theVehicle]) then
        return false
    end
...
function setVehicleDirtEnabled(theVehicle, enabledState)
    assert(isElement(theVehicle) and getElementType(theVehicle) == "vehicle", "Bad argument @ 'setVehicleDirtEnabled' [Expected vehicle element at argument 1, got "..(isElement(theVehicle) and getElementType(theVehicle) or type(theVehicle)).."]")
    assert(type(enabledState) == "boolean", "Bad argument @ 'setVehicleDirtEnabled' [Expected boolean at argument 2, got "..(type(enabledState)).."]")

    if not enabledState then
        if not isElement(enabledVehicles[theVehicle]) then
            enabledVehicles[theVehicle] = dxCreateShader(shaderCode, 0, 100, false, "vehicle")
            if isElement(enabledVehicles[theVehicle]) then
                engineApplyShaderToWorldTexture(enabledVehicles[theVehicle], "vehiclegrunge256", theVehicle)
            else
                return false
            end
        end
        ...
        if isElement(enabledVehicles[theVehicle]) then
            engineRemoveShaderFromWorldTexture(enabledVehicles[theVehicle], "vehiclegrunge256", theVehicle)
            destroyElement(enabledVehicles[theVehicle])
            enabledVehicles[theVehicle] = nil
        end
    end
...
addEventHandler("onClientElementDestroy", root, function()
    if getElementType(source) == "vehicle" then
        setVehicleDirtEnabled(source, true)
    end
end)
```
...
```
--// Disable dirt on the localPlayer's vehicle
local theVeh = getPedOccupiedVehicle(localPlayer)
if theVeh then
    setVehicleDirtEnabled(theVeh, false)
end

--// Show the dirt on a vehicle after 5 seconds
setTimer(function()
    if theVeh then
        setVehicleDirtEnabled(theVeh, true)
    end
end, 5000, 1)

--// Also u can use 'isVehicleDirtEnabled' to check.
```
...
- setVehicleDirtEnabled » This function toggles a dirt removal shader effect on the specified vehicle.
