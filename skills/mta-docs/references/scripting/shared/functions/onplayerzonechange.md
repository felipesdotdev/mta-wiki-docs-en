---
doc_id: "mta-wiki:14159"
title: "OnPlayerZoneChange"
source_title: "OnPlayerZoneChange"
source_url: "https://wiki.multitheftauto.com/wiki/OnPlayerZoneChange"
revision_id: 82605
language: "en"
...
This code implements an [event](https://wiki.multitheftauto.com/index.php?search=event) that is triggered when the [player](https://wiki.multitheftauto.com/index.php?search=player) enters a new area on the map.
...
- **oldZone:** old zone of [player](https://wiki.multitheftauto.com/index.php?search=player).

- **newZone:** new zone of [player](https://wiki.multitheftauto.com/index.php?search=player).

## Source

The [source](mta://reference/misc/event-system.md) of this event is the player whose zone was changed.
...
```
local playersZone = {}
local zoneEventName = (localPlayer and "onClientPlayerZoneChange" or "onPlayerZoneChange") -- client = "onClientPlayerZoneChange" — server = "onPlayerZoneChange"
local zoneCitiesOnly = false -- only major cities?

local function checkPlayersZone()
    ...
    for playerID = 1, #playersTable do
        ...
        local playerZoneNow = getZoneName(playerX, playerY, playerZ, zoneCitiesOnly)
        local playerZoneSaved = playersZone[playerElement]
        local playerZoneNeedsUpdate = (playerZoneSaved and playerZoneSaved ~= playerZoneNow)

        if (playerZoneNeedsUpdate) then
            triggerEvent(zoneEventName, playerElement, playerZoneSaved, playerZoneNow)
        end

        playersZone[playerElement] = playerZoneNow
    end
end
addEvent(zoneEventName, false) -- disable remote triggering so players couldn't trigger this event on server (& vice-versa)
setTimer(checkPlayersZone, 500, 0)

local function clearPlayersZone()
    playersZone[source] = nil
end
addEventHandler(localPlayer and "onClientPlayerQuit" or "onPlayerQuit", root, clearPlayersZone)
```
...
This example will make label with the full zone name of the player, **onPlayerZoneChange** is function is change the label text to new zone when the player enter a new zone.

Click to collapse [-]
Client

```
local myCurrentZoneLabel

addEventHandler("onClientResourceStart", resourceRoot, function()
    local px, py, pz = getElementPosition(localPlayer)
    local firstZone = getZoneName(px, py, pz)
    myCurrentZoneLabel = guiCreateLabel(0, 0, 0.20, 0.02, firstZone, true)
end)

addEventHandler("onClientPlayerZoneChange", root, function(_, newZone)
    if (source == localPlayer) then
        guiSetText(myCurrentZoneLabel, newZone)
    end
end)
```

This example will out put in the chat the new zone full name when the player enter a new zone.

Click to collapse [-]
Server

```
addEventHandler("onPlayerZoneChange", root, function(oldZone, newZone)
    outputChatBox("Your Zone is Changed from '"..(oldZone).."' to '"..(newZone).."'.", source, 255, 255, 0)
end)
```
