...
This function will add an [event](https://wiki.multitheftauto.com/index.php?search=event) handler. An event handler is a function that will be called when the event it's attached to is triggered. See [event system](mta://reference/misc/event-system.md) for more information on how the event system works.
...
- **eventName:** The name of the [event](https://wiki.multitheftauto.com/index.php?search=event) you want to attach the handler function to. **Note: The maximum allowed length is 100 ASCII characters (that is, English letters and numerals)**
