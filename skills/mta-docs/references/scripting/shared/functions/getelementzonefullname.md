---
doc_id: "mta-wiki:14161"
title: "GetElementZoneFullName"
source_title: "GetElementZoneFullName"
source_url: "https://wiki.multitheftauto.com/wiki/GetElementZoneFullName"
revision_id: 82150
language: "en"
...
This function allows you to retrieve the zone full name of a [element](mta://reference/misc/element.md), This can be [player](https://wiki.multitheftauto.com/index.php?search=player), [ped](https://wiki.multitheftauto.com/index.php?search=ped), [object](https://wiki.multitheftauto.com/index.php?search=object), [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle), [weapon](https://wiki.multitheftauto.com/index.php?search=weapon) or other [element](mta://reference/misc/element.md).

## Syntax

```
string getElementZoneFullName( element theElement )
```

### Required Arguments

- **theElement**: the [element](mta://reference/misc/element.md) to get its full zone name.

### Returns

Returns the full name of the element zone.
...
```
function getElementZoneFullName(theElement)
    assert(isElement(theElement), "Bad argument @ 'getElementZoneFullName' [Expected element at argument 1, got "..(type(theElement)).."]")
    local x, y, z = getElementPosition(theElement)
    local city, zone = getZoneName(x, y, z, true), getZoneName(x, y, z)
    return (city == zone and city or city..", "..zone)
end
```
...
This example will make label with the full zone name of the player.

*NOTE:* This function requires function **[onPlayerZoneChange](mta://scripting/shared/functions/onplayerzonechange.md)** in order to work correctly.
...
addEventHandler("onClientResourceStart", resourceRoot, function()
    local text = getElementZoneFullName(localPlayer)
    myCurrentZoneLabel = guiCreateLabel(0, 0, 0.20, 0.02, text, true)
end)

addEventHandler("onClientPlayerZoneChange", root, function()
    if (source == localPlayer) then
    	local newText = getElementZoneFullName(localPlayer)
        guiSetText(myCurrentZoneLabel, newText)
    end
end)
```
