---
doc_id: "mta-wiki:12162"
title: "GetPedWeaponSkill"
source_title: "GetPedWeaponSkill"
source_url: "https://wiki.multitheftauto.com/wiki/GetPedWeaponSkill"
revision_id: 68934
language: "en"
...
This function gets the [ped](https://wiki.multitheftauto.com/index.php?search=ped)'s corresponding [weapon skill level](mta://reference/misc/weapon-skill-levels.md)

## Syntax

```
string getPedWeaponSkill ( ped thePed, int weaponID/string weaponName )
```

### Required Arguments

- **thePed:** the [ped](https://wiki.multitheftauto.com/index.php?search=ped) whose weapon skill level you want to get
...
A *string* with the [ped](https://wiki.multitheftauto.com/index.php?search=ped)'s corresponding [weapon skill level](mta://reference/misc/weapon-skill-levels.md) name or *false* if the passed arguments were invalid
...
The next code snippet adds a /skilllevel command, which allows player to check his weapon skill level

```
function skilllevelCommand()
    local weaponID = getPedWeapon(localPlayer)
    if weaponID == 0 then
        outputChatBox("You have no weapon")
        return false
    end
    local levelName = getPedWeaponSkill(localPlayer, weaponID)
    if not levelName then
        outputChatBox("This weapon has no its skill level")
        return false
    end
    outputChatBox("Your skill level is: "..levelName.."!")
end
addCommandHandler("skilllevel", skilllevelCommand)
```
