---
doc_id: "mta-wiki:3971"
title: "GetPedWeapon"
source_title: "GetPedWeapon"
source_url: "https://wiki.multitheftauto.com/wiki/GetPedWeapon"
revision_id: 74105
language: "en"
...
This function tells you which weapon type is in a certain **[weapon slot](https://wiki.multitheftauto.com/index.php?search=weapon%20slot)** of a ped.
...
**Method**: *[ped](https://wiki.multitheftauto.com/index.php?search=ped):getWeapon(...)*

### Required Arguments

- **thePed**: the [ped](https://wiki.multitheftauto.com/index.php?search=ped) you want to get the weapon type from.

### Optional Arguments

- **weaponSlot**: an integer representing the [weapon slot](https://wiki.multitheftauto.com/index.php?search=weapon%20slot) (set to the ped's current slot if not given).

### Returns

Returns an [int](mta://reference/misc/int.md) indicating the type of the weapon the ped has in the specified slot. If the slot is empty, it returns 0.

It should be noted that if a ped runs out of ammo for a weapon, it will still return the ID of that weapon in the slot (even if it appears as if the ped does not have a weapon at all), though [getPedTotalAmmo](mta://scripting/shared/functions/getpedtotalammo.md) will return **0**.  Therefore, [getPedTotalAmmo](mta://scripting/shared/functions/getpedtotalammo.md) should be used in conjunction with getPedWeapon in order to check if a ped has a weapon.
