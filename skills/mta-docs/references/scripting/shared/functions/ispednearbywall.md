---
doc_id: "mta-wiki:14183"
title: "IsPedNearbyWall"
source_title: "IsPedNearbyWall"
source_url: "https://wiki.multitheftauto.com/wiki/IsPedNearbyWall"
revision_id: 82618
language: "en"
...
This function checks if [player](https://wiki.multitheftauto.com/index.php?search=player)/[ped](https://wiki.multitheftauto.com/index.php?search=ped) is nearby walls like buildings or objects.

| [[{{{image}}}\|link=\|]] | Note: This function check if there are walls like buildings or objects in player / ped 's front angle. |
| --- | --- |
|  |  |
...
```
bool isPedNearbyWall( element element, [ int customDistance = 2 ] )
```

### Required Arguments

- **element**: the [player](https://wiki.multitheftauto.com/index.php?search=player)/[ped](https://wiki.multitheftauto.com/index.php?search=ped) element to check.

### Optional Arguments

*NOTE:* When using optional arguments, you might need to supply all arguments before the one you wish to use. For more information on optional arguments, see [optional arguments](https://wiki.multitheftauto.com/index.php?search=optional%20arguments).

- **customDistance**: the distance between the walls and the [player](https://wiki.multitheftauto.com/index.php?search=player)/[ped](https://wiki.multitheftauto.com/index.php?search=ped).

### Returns

- *true* if the player/ped is nearby a wall.
...
```
function isPedNearbyWall(element, customDistance)
    assert(isElement(element) and (getElementType(element) == "player" or getElementType(element) == "ped"), "Bad argument @ 'isPedNearbyWall' [Expected player/ped element at argument 1, got "..(isElement(element) and getElementType(element) or type(element)).."]")
    local px, py, pz = getElementPosition(element)
    local rz = select(3, getElementRotation(element))
    local _px = px - math.sin(math.rad(rz)) * (customDistance or 2)
    local _py = py + math.cos(math.rad(rz)) * (customDistance or 2)
    if (not isLineOfSightClear(px, py, pz, _px, _py, pz, true, false, false, true, false)) then
        return true
    end
    return false
end
...
```
addCommandHandler("nearbywall", function()
    if isPedNearbyWall(localPlayer) then
        outputChatBox("* You are nearby a wall currently.", 0, 255, 0)
    else
        outputChatBox("* You are not nearby any wall.", 255, 0, 0)
    end
end)
```

```
local myPed = createPed(0, 2105.46, -1582.98, 26.11, 270)
setPedControlState(myPed, "forwards", true)

local myTimer
myTimer = setTimer(function()
    if isPedNearbyWall(myPed, 3) then
        killTimer(myTimer)
        setPedControlState(myPed, "forwards", false)
        outputChatBox("There is a wall next to the ped, the ped stopped moving.")
    end
end, 250, 0)
```
