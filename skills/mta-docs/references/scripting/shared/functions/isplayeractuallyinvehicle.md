---
doc_id: "mta-wiki:13863"
title: "IsPlayerActuallyInVehicle"
source_title: "IsPlayerActuallyInVehicle"
source_url: "https://wiki.multitheftauto.com/wiki/IsPlayerActuallyInVehicle"
revision_id: 76056
language: "en"
categories: ["Useful_Functions"]
---

# IsPlayerActuallyInVehicle

This function provides a hacky way to recreate the client-side behavior of [isPedInVehicle](mta://scripting/shared/functions/ispedinvehicle.md) server-sided for [player](https://wiki.multitheftauto.com/index.php?search=player) elements ONLY. It checks if a specified [player](https://wiki.multitheftauto.com/index.php?search=player) is actually in a [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle) and not just in the process of entering.

## Syntax

```
bool isPlayerActuallyInVehicle ( player thePlayer )
```

### Required Arguments

- **thePlayer**: a [player](https://wiki.multitheftauto.com/index.php?search=player) to check if they are inside a [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle) or not.

### Returns

Returns *true* if the specified [player](https://wiki.multitheftauto.com/index.php?search=player) has fully completed entering a [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle) and hasn't fully completed exiting it yet, *false* otherwise.
...
```
function isPlayerActuallyInVehicle(player)
	assert(isElement(player) and (getElementType(player) == "player"), "Bad argument @ isPlayerActuallyInVehicle [player expected, got " .. type(player) .. " '" .. tostring(player) .. "']")
...
This example will output a chat message anywhere within 50ms of a player changing their in-vehicle-state according to isPlayerActuallyInVehicle.

```
local allPlayers = {}
function resStart()
	for i,v in ipairs(getElementsByType("player")) do
		allPlayers[v] = isPlayerActuallyInVehicle(v)
	end
	setTimer(checkInterval,50,0)
end
addEventHandler("onResourceStart",getResourceRootElement(getThisResource()),resStart)

function addPlayer()
	allPlayers[source] = isPlayerActuallyInVehicle(source)
end
addEventHandler("onPlayerJoin",getRootElement(),addPlayer)

function checkInterval()
	for k,v in pairs(allPlayers) do
		local result = isPlayerActuallyInVehicle(k)
		if not v == result then
			outputChatBox(getPlayerName(k).." changed their in vehicle state to "..tostring(result))
			allPlayers[k] = result
		end
	end
end
...
Author: [User:Dragon](https://wiki.multitheftauto.com/index.php?title=User:Dragon&action=edit&redlink=1)
