---
doc_id: "mta-wiki:9311"
title: "GetPlayerFromAccountName"
source_title: "GetPlayerFromAccountName"
source_url: "https://wiki.multitheftauto.com/wiki/GetPlayerFromAccountName"
revision_id: 78389
language: "en"
...
This function get online [player](https://wiki.multitheftauto.com/index.php?search=player) from their account name.
...
- [player](https://wiki.multitheftauto.com/index.php?search=player) element if the account name exists.

- *false* if the player is not found.
...
```
addCommandHandler("getPlayerName", function(player, _, name)
    if name then
        local thePlayer = getPlayerFromAccountName(name)
        if thePlayer then
            outputChatBox(getPlayerName(thePlayer).."'s the owner of account "..name, player, 255, 255, 255, true)
        else
            outputChatBox("player not found!", player, 255, 0, 0)
        end
    else
        outputChatBox("type account name first!", player, 255, 255, 0)
    end
end)
```
