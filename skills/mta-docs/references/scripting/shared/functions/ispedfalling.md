---
doc_id: "mta-wiki:14564"
title: "IsPedFalling"
source_title: "IsPedFalling"
source_url: "https://wiki.multitheftauto.com/wiki/IsPedFalling"
revision_id: 82650
language: "en"
...
This function checks if [player](https://wiki.multitheftauto.com/index.php?search=player)/[ped](https://wiki.multitheftauto.com/index.php?search=ped) is falling from a high place.

## Syntax

```
bool isPedFalling( element thePed )
```

### Required Arguments

- **thePed**: the [player](https://wiki.multitheftauto.com/index.php?search=player)/[ped](https://wiki.multitheftauto.com/index.php?search=ped) element to check.

### Returns

- *true* if the player/ped is falling.
...
```
function isPedFalling(thePed)
    if not isElement(thePed) or (getElementType(thePed) ~= "player" and getElementType(thePed) ~= "ped") then
        return false
    end

    if isPedOnGround(thePed) or isPedInVehicle(thePed) or getPedOccupiedVehicle(thePed) then
        return false
    end

    local vz = select(3, getElementVelocity(thePed))
    local verticalSpeed = vz * 50
...
addEventHandler("onClientRender", root, function()
    if isPedFalling(localPlayer) then
        dxDrawText("You are falling!", 0, 0, screenX, screenY, tocolor(255, 255, 255, 255), 1, "default-bold", "center", "center")
    end
end)
```
