...
Returns a multi-dimensional array (which can be transformed into a proper [matrix](mta://reference/misc/matrix.md) class using *Matrix.create* method) containing a 4x4 matrix. Returns *false* if the element is not streamed in, and not a [vehicle](https://wiki.multitheftauto.com/index.php?search=vehicle), [ped](https://wiki.multitheftauto.com/index.php?search=ped) or [object](https://wiki.multitheftauto.com/index.php?search=object).
