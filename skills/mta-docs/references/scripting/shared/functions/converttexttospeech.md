...
- If called clientside and if all arguments are valid, this function returns *true*, the corresponding [sound](https://wiki.multitheftauto.com/index.php?search=sound) element and its URL if the sound could theorically be played (althrough it may not play); *[nil](mta://reference/misc/nil.md)* otherwise.
