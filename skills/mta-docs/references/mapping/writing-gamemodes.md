---
doc_id: "mta-wiki:3465"
title: "Writing Gamemodes"
source_title: "Writing Gamemodes"
source_url: "https://wiki.multitheftauto.com/wiki/Writing_Gamemodes"
revision_id: 65894
language: "en"
...
This guide tries to outline the process of how to write a proper gamemode. If you just started with scripting for MTA, you may want to check the other scripting tutorials at the [Main Page](mta://reference/misc/main-page.md) first.
...
The [getElementsByType](mta://scripting/shared/functions/getelementsbytype.md) function retrieves a table of all the elements of a certain type (the type corresponds to the node name in the .map file). This works for both custom types and built-in MTA types (like "vehicle" or "player").
[getElementData](mta://scripting/shared/functions/getelementdata.md) can be used to retrieve the xml attributes set in the .map file.
...
Now instead of writing a map-loader for every single gamemode, the [Map manager](https://wiki.multitheftauto.com/index.php?search=Map%20manager) provides functions to load gamemodes and maps. Simply put, when you enter the correct command (for example 'gamemode ctf ctf-italy') it will start both resources 'ctf' and 'ctf-italy' while triggering an event ([onGamemodeMapStart](https://wiki.multitheftauto.com/index.php?title=OnGamemodeMapStart&action=edit&redlink=1)) to tell the 'ctf' resource that a map was loaded. The 'ctf' resource can then access the information 'ctf-italy' contains and start spawning players etc.
...
[Map manager](https://wiki.multitheftauto.com/index.php?search=Map%20manager) exports a few more access functions which you don't have to use, but may be useful.
...
Read the [helpmanager help page](mta://resources/helpmanager.md) for details on how to do it.

### Scoreboard

The scoreboard displays players and teams currently in-game. You add custom columns to it to provide map specific information. For example the column 'points' in the 'ctf' gamemode could represent the player's points gained through kills or captures. As usual, see the [scoreboard help page](mta://resources/dxscoreboard.md) for more information.
