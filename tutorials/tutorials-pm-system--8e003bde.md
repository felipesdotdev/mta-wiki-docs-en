---
doc_id: "mta-wiki:7691"
title: "Making a PM System"
source_title: "Tutorials:PM System"
source_url: "https://wiki.multitheftauto.com/wiki/Tutorials%3APM_System"
revision_id: 59161
language: "en"
categories: ["Tutorials"]
generated_at: "2026-07-26T16:17:01.194865+00:00"
---
...
XML is another language, so I won't teach you much about it. But to learn more about the meta.xml file, visit [this page](mta://reference/misc/meta-xml.md). So type the following code into the file.
...
Creating command handlers are the way to creating commands in MTA, you won't have to monitor chat input and manually separate text - it's all nicely put into one nice function. First we have to create a function, and then create a command which will bind itself to the function. To do this we use [addCommandHandler](mta://scripting/shared/functions/addcommandhandler.md), so let's start off with this code:
...
We won't check if a player exists, that's too troublesome. Instead we'll resort to a simple function that we know will return [nil](mta://reference/misc/nil.md) (nothing). When functions return something, it's like when you ask someone a question and they reply back with a definite answer when they think and answer your question or reply to your order. "That is false, sir", "That is true, sir!" and "Mission failed, sir!". We'll be using [getPlayerFromName](mta://scripting/shared/functions/getplayerfromname.md). This function takes one argument, the name of the player we want and it returns to kinds of values: [nil](mta://reference/misc/nil.md) (nothing) and a [Player](mta://reference/misc/player.md) [element](mta://reference/misc/element.md). It will return nil if it couldn't find a player with that exact name (ignoring capital letters) or it will return a player element if it could find a match. So let's add that to our code:
