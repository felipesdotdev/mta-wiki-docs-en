---
doc_id: "mta-wiki:5364"
title: "Resource : Performancebrowser"
source_title: "Resource:Performancebrowser"
source_url: "https://wiki.multitheftauto.com/wiki/Resource%3APerformancebrowser"
revision_id: 74603
language: "en"
categories: ["Resource"]
generated_at: "2026-07-26T16:16:59.137428+00:00"
---
...
A performance browser is used to monitor the performance of resources on the server. Specifically, it shows how much CPU time and memory each resource is consuming. It is helpful in finding out which resources are slow, and which ones are leaking memory.
...
Then all you have to do is add users to the **"DevGroup"** and use this URL in a browser: **http://SERVER_IP:HTTP_PORT/performancebrowser/**
On localhost it would look something like this: **http://localhost:22005/performancebrowser/**. So, turns out the **default HTTP port is 22005.**

# In game access

There is also an in-game version of the performance browser called '**ipb'** which is now part of the official server resources. If you don't have it, it can be found in the latest resources zip: [http://mirror.mtasa.com/mtasa/resources/](http://mirror.mtasa.com/mtasa/resources/)

To access the IPB, log in to an account with access to admin permissions then use the command **/ipb**.

# Category - Event Packet usage

Select **Event Packet usage** from the **Category** drop down to view details about [triggerClientEvent](mta://scripting/server/functions/triggerclientevent.md) and outgoing [setElementData](mta://scripting/shared/functions/setelementdata.md) packet usage.

##### The main columns are:

- **Type**        - How the packets are created. This can be one or more of:

- *ElementData* - Caused by server side [setElementData](mta://scripting/shared/functions/setelementdata.md)

- *ElementData(Relay)* - Caused by client side [setElementData](mta://scripting/shared/functions/setelementdata.md)

- *Event* - Caused by server side [triggerClientEvent](mta://scripting/server/functions/triggerclientevent.md)
...
- **Raknet thread** - handles the sending and receiving network packets to/from the connected clients.

- **DB thread** - is for database stuff.

The total % available for MTA depends on the number of CPU cores available. If MTA has access to 4 CPU cores, then the max CPU available is 400%, (although each individual thread cannot go above 100%). If a **Sys:** amount is shown, this is an additional % used by the system (O/S) for doing its job.
